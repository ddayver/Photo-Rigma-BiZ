...
**	Decr.:	Вывод, редактировани, загрузка и обработка изображений, оценок	**
*****************************************************************************/
...
include_once($config['site_dir'] . 'language/' . $config['language'] . '/photo.php'); // подключаем языковый файл изображений
include_once($config['site_dir'] . 'language/' . $config['language'] . '/category.php'); // подключаем языковый файл разделов
...
else // иначе
{
	...
	{
		if(!@fopen($config['site_dir'] . $config['gallery_folder'] . '/' . $temp2['folder'] . '/' . $temp['file'], 'r')) // проверяем, существует ли файл изображения, если не существует, то...
		{
            $photo_id = 0; // установить идентификатор выводимого изоюражения равным 0
		}
	}
...
$max_photo_w = $config['max_photo_w']; // получаем данные о максимальной ширине изображения
$max_photo_h = $config['max_photo_h']; // и о максимальной высоте
...
if ($photo_id != 0) // если существует идентификатор изображения (не равен 0)
{
	...
	elseif (isset($_REQUEST['subact']) && $_REQUEST['subact'] == 'saveedit' && (($temp_foto['user_upload'] == $user->user['id'] && $user->user['id'] != 0) || $user->user['pic_moderate'] == true)) // если поступила команда на сохранение изменений в изображении и пользователь имеет право редактировать изображение или является его автором, то...
	{
		...
		if($category && $temp_foto['category'] != $_POST['name_category']) // если смена раздела разрешена и новый раздел не является таким же, как и был, то...
		{
			...
			$path_old_photo = $config['site_dir'] . $config['gallery_folder'] . '/' . $temp_old['folder'] . '/' . $temp_foto['file']; // формируем старый путь к изображению
			$path_new_photo = $config['site_dir'] . $config['gallery_folder'] . '/' . $temp_new['folder'] . '/' . $temp_foto['file']; // формируем новый путь к изображению
			$path_old_thumbnail = $config['site_dir'] . $config['thumbnail_folder'] . '/' . $temp_old['folder'] . '/' . $temp_foto['file']; // формируем старый путь к эскизу
			$path_new_thumbnail = $config['site_dir'] . $config['thumbnail_folder'] . '/' . $temp_new['folder'] . '/' . $temp_foto['file']; // формируем новый путь к эскизу
	...
	if (isset($_REQUEST['subact']) && $_REQUEST['subact'] == "edit" && (($temp_foto['user_upload'] == $user->user['id'] && $user->user['id'] != 0) || $user->user['pic_moderate'] == true)) // если поступила команда на редактирование изображения и пользователь является владельцем изображения и не является гстем или пользователь имеет право редактирования изображений, то формируем блок редактирования изображения
	{
		...
		$photo['path'] = $config['site_dir'] . $config['thumbnail_folder'] . '/' . $temp_category['folder'] . '/' . $temp_foto['file']; // формируем путь к эскизу изображения
		$photo['url'] = $config['site_url'] . '?action=attach&foto=' . $temp_foto['id'] . '&thumbnail=1'; // формируем ссылку для вывода эскиза изображения
		...
		if ($user_add) // если пользователь существует, то...
		{
			...
			$photo['user_url'] = $config['site_url']  . '?action=login&subact=profile&uid=' . $temp_foto['user_upload']; // и формируем ссылку на профиль
		}
		else // иначе
		{
		...
		$photo['category_name'] .= '</select>'; // закрываем список разделов
   	    $photo['url_edited'] = $config['site_url'] . '?action=photo&subact=saveedit&id=' . $temp_foto['id']; // ссылка для сохранения изменений
       	$photo['url_edited_text'] = $lang['photo_save']; // надпись для кнопки Сохранить
		$max_photo_w = $config['temp_photo_w']; // изменяем максимальную высоту изображения на максимальную высоту эскиза
		$max_photo_h = $config['temp_photo_h']; // и аналогично для ширины
	}
	elseif (isset($_REQUEST['subact']) && $_REQUEST['subact'] == "delete" && (($temp_foto['user_upload'] == $user->user['id'] && $user->user['id'] != 0) || $user->user['pic_moderate'] == true)) // если поступила команда на удаление изображения и пользователь является владельцем изображения и не является гостем или пользователь имеет право редактирования изображений, то производим процедуру удаления
	{
		...
		if($temp_category['id'] == 0) // если удаляется файл из пользовательского альбома, формируем ссылку для редиректа...
		{
			...
			{
				$photo['category_url'] = $config['site_url'] . '?action=category&cat=user&id=' . $temp_foto['user_upload']; // ссылка на редирект будет отправлять в этот пользовательский альбом
			}
			else // иначе
			{
				...
				if($temp > 0) // если да, то...
				{
					$photo['category_url'] = $config['site_url'] . '?action=category&cat=user'; // редирект произойдет к списку пользовательских альбомов
				}
				else // иначе
				{
					...
					if($temp > 0) // если да, то...
					{
						$photo['category_url'] = $config['site_url'] . '?action=category'; // редирект произойдет к списку разделов
					}
					else // иначе
					{
						$photo['category_url'] = $config['site_url']; // редирект будет на главную страницу сайта
					}
				}
			}
		}
		else // иначе если удаление НЕ из пользовательского раздела
		{
			...
			if($temp > 0) // если да, то...
			{
				$photo['category_url'] = $config['site_url'] . '?action=category&cat=' . $temp_category['id']; // редирект в данный раздел
			}
			else // иначе
			{
				...
				if($temp > 0) // если да, то...
				{
					$photo['category_url'] = $config['site_url'] . '?action=category'; // редирект к спску разделов
				}
				else // иначе
				{
					$photo['category_url'] = $config['site_url']; // редирект на главную страницу сайта
				}
			}
		}
		...
		{
			$redirect_url = $config['site_url'] . '?action=photo&id=' . $photo_id; // редирект на не удачно удаленное изображение
			$redirect_time = 5; // устанавливаем время редиректа 5 сек
	...
	else // если не поступило никаких команд, то формируем вывод изображения
	{
		$temp_foto = $db->fetch_array("SELECT * FROM `photo` WHERE `id` = " . $photo_id); // полчаем данные об изображении
    	$temp_category = $db->fetch_array("SELECT * FROM `category` WHERE `id` = " .  $temp_foto['category']); // получаем данные о разделе изображения
		$photo['path'] = $config['site_dir'] . $config['gallery_folder'] . '/' . $temp_category['folder'] . '/' . $temp_foto['file']; // формируем путь к изображению
		$photo['url'] = $config['site_url'] . '?action=attach&foto=' . $temp_foto['id']; // формируем ссылку для вывода изображения
		$photo['name'] = $temp_foto['name']; // сохраняем название изображения
		$photo['description'] = $temp_foto['description']; // сохраняем описание изображения
		...
		{
    	    $photo['url_edit'] = $config['site_url'] . '?action=photo&subact=edit&id=' . $temp_foto['id']; // ссылка на редактирование
        	$photo['url_edit_text'] = $lang['photo_edit']; // текст Редактировать
    	    $photo['url_delete'] = $config['site_url'] . '?action=photo&subact=delete&id=' . $temp_foto['id']; // ссылка на удаление
		...
		{
			$photo['user'] = $user_add['real_name']; // сохраняем отображаемое имя
			$photo['user_url'] = $config['site_url']  . '?action=login&subact=profile&uid=' . $temp_foto['user_upload']; // и ссылку на профиль автора
		}
		else // иначе
		{
		...
		if($temp_category['id'] == 0) // если раздел является пользовательским альбомом, то...
		{
			...
			$photo['category_url'] = $config['site_url'] . '?action=category&cat=user&id=' . $temp_foto['user_upload']; // сохраняем ссылку на пользовательский альбом
		}
		else // иначе
		{
			$photo['category_url'] = $config['site_url'] . '?action=category&cat=' . $temp_category['id']; // сохраняем ссылку на обычный раздел
		}
		...
		if (($user->user['pic_rate_user'] == true || $user->user['pic_rate_moder'] == true) && $temp_foto['user_upload'] != $user->user['id']) // если пользователь имеет право оценивать изображения как пользователь или как преподаватель (модератор) и не является автором изображения, то...
		{
			...
			{
        	    $photo['rate_you_url'] = $config['site_url'] . '?action=photo&subact=rate&id=' . $photo_id; // сохраняем ссылку для оценки изображения
...
else // иначе если не указан был идентификатор изображения, то...
{
	if (isset($_REQUEST['subact']) && $_REQUEST['subact'] == "upload" && $user->user['id'] != 0 && $user->user['pic_upload'] == true) // если получена команда на загрузку изображения и пользователь имеет право загрузки изображения, то...
	{
		...
		$photo['path'] = $config['site_dir'] . $config['gallery_folder'] . '/' . $temp_foto['file']; // и путь к нему
		...
		$max_size = $work->return_bytes($config['max_file_size']); // получаем максимально разрешаемый размер файла для заливки в настройках сайта (в байтах)
		...
		$photo['category_name'] .= '</select>'; // закрываем список разделов
   	    $photo['url_uploaded'] = $config['site_url'] . '?action=photo&subact=uploaded'; // указываем ссылку для заливки изображения
	}
	elseif (isset($_REQUEST['subact']) && $_REQUEST['subact'] == "uploaded" && $user->user['id'] != 0 && $user->user['pic_upload'] == true) // инче если получена команда на сохранение загруженного изображения и пользователь имеет право загрузки изображения, то...
	{
		...
		if($submit_upload) // если заливка еще разрешена, то...
		{
			...
			if ($_FILES['file_photo']['error'] == 0 && $_FILES['file_photo']['size'] > 0 && $_FILES['file_photo']['size'] <= $max_size && mb_eregi('(gif|jpeg|png)$', $_FILES['file_photo']['type'])) // проверяем, что файл изображения загружен без ошибок, размер и тип файла соотвествуют настройкам, если да, то...
			{
				...
				$photo['path'] = $config['site_dir'] . $config['gallery_folder'] . '/' . $temp_category['folder'] . '/' . $file_name; // формируем путь к изображению
				$photo['thumbnail_path'] = $config['site_dir'] . $config['thumbnail_folder'] . '/' . $temp_category['folder'] . '/' . $file_name; // формируем путь к эскизу
		...
		if($submit_upload) // если загрузка успешна, то...
		{
			...
			{
				$redirect_url = $config['site_url'] . '?action=photo&id=' . $photo_id; // ссылка редиректа будет вести на страницу просмотра загруженного изобрадения
				$redirect_time = 5; // устанавливаем время редиректа 5 сек
			...
			else // иначе, если запись в базу не удалась
			{
				...
				$redirect_url = $config['site_url'] . '?action=photo&subact=upload'; // редирект ведет на форму загрузки изображения
		...
		{
			$redirect_url = $config['site_url'] . '?action=photo&subact=upload'; // редирект на страницу загрузки изображения
			$redirect_time = 3; // устанавливаем время редиректа 3 сек
			$redirect_message = $lang['photo_error_upload']; // сообщаем о неудачной загрузке
		}
	}
	else // иначе если данные об изображении получить не удалось, формируем вывод для отсутствующего изображения
	{
		...
		$photo['path'] = $config['site_dir'] . $config['gallery_folder'] . '/' . $temp_foto['file']; // путь к файлу
		$photo['url'] = $config['site_url'] . '?action=attach&foto=0'; // ссылку на несуществующее изображение
		...
		$photo['category_url'] = $config['site_url']; // вместо ссылки на раздел - ссылка на главную страницу сайта
...
if ((isset($_REQUEST['subact']) && $_REQUEST['subact'] == "uploaded" && $user->user['id'] != 0 && $user->user['pic_upload'] == true) || (isset($_REQUEST['subact']) && $_REQUEST['subact'] == "delete" && (($temp_foto['user_upload'] == $user->user['id'] && $user->user['id'] != 0) || $user->user['pic_moderate'] == true))) // если были команды, требующие после себя редиректа (сохранение загруженного изображения или его удаление), то формируем блок редиректа
{
	...
	$title = $config['title_name'] . ' - ' . $lang['main_redirect_title']; // дополнительным заголовком будет сообщение о переадресации
