 ...
 *
 * @section   Main_Main_Functions Основные функции
 ...
 *
 * @section   Main_Error_Handling Обработка ошибок
 *            При возникновении ошибок генерируются исключения. Поддерживаемые типы исключений:
 *            - `RuntimeException`: Если возникают ошибки при получении данных пользователя из базы данных.
 *            - `RuntimeException`: Если пользователь не имеет прав на просмотр новостей.
 ...
 *                             Пример сообщения:
 *                             "Ошибка базы данных | Не удалось найти пользователя с ID: [ID пользователя]".
 * @throws    RuntimeException Если пользователь не имеет прав на просмотр новостей.
 *                             Пример сообщения:
 *                             "Пользователь не имеет прав на просмотр новостей | ID: {$user->user['id']}".
 * @throws    Exception        При выполнении прочих методов классов, функций или операций.
 *
 * @section   Main_Related_Files Связанные файлы и компоненты
 *            - Классы приложения:
 ...
 *            - Файлы приложения:
 *              - @see index.php Этот файл подключает action/main.php по запросу из `$_GET`.
 *
