...
if ($cat == 'user' || $cat === 0) {
    ...
    if ($work->check_get('subact', true, true) && $_GET['subact'] == 'saveedit' && $user->user['cat_moderate'] == true && $cat != 0) {
        if ($db->select('*', TBL_CATEGORY, '`id` = ' . $cat)) {
            ...
            if ($temp) {
                ...
                if (!$db->update(array('name' => $name_category, 'description' => $description_category), TBL_CATEGORY, '`id` = ' . $cat)) {
                    log_in_file($db->error, DIE_IF_ERROR);
                }
            } else {
    ...
    if ($work->check_get('subact', true, true) && $_GET['subact'] == 'add' && $user->user['cat_moderate'] == true) {
        ...
        if ($db->select('COUNT(*) AS `count_dir`', TBL_CATEGORY, '`folder` = \'' . $name_dir . '\'')) {
            $temp = $db->res_row();
            if ((isset($temp['count_dir']) && $temp['count_dir'] > 0) || is_dir($work->config['site_dir'] . $work->config['gallery_folder'] . '/' . $name_dir) || is_dir($work->config['site_dir'] . $work->config['thumbnail_folder'] . '/' . $name_dir)) {
        ...
        if (!$work->check_post('name_category', true, true)) {
            $name_category = $lang['category']['no_name'] . ' (' . $name_dir . ')';
        } else {
            $name_category = $_POST['name_category'];
        }

        if (!$work->check_post('description_category', true, true)) {
            $description_category = $lang['category']['no_description'] . ' (' . $name_dir . ')';
        } else {
            $description_category = $_POST['description_category'];
        }
