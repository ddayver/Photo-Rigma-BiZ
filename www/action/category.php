...
include_once($work->config['site_dir'] . 'language/' . $work->config['language'] . '/category.php'); // подключаем языковый файл категорий
...
elseif (mb_ereg('^[0-9]+$', $cat)) // иначе если запрошен определенный раздел (идентификатор раздела равен числу - к примеру, cat=1)
{
	...
	elseif (isset($_REQUEST['subact']) && $_REQUEST['subact'] == 'delete' && $user->user['cat_moderate'] == true && $cat != 0) // иначе если поступила под-команда о необходимости удалить раздел, пользователь имеет право удалить раздел и раздел не равен 0 (не является пользовательскими альбомами), то...
	{
		...
		if($temp) // если раздел существует, то...
		{
			...
			$cat_dir = dir($work->config['site_dir'] . $work->config['gallery_folder'] . '/' . $temp['folder']); // делаем дополнительную чистку папки раздела - открываем папку раздела
			...
			$cat_dir = dir($work->config['site_dir'] . $work->config['thumbnail_folder'] . '/' . $temp['folder']); // делаем дополнительную чистку папки эскизов раздела - открываем папку эскизов раздела
			...
			rmdir($work->config['site_dir'] . $work->config['gallery_folder'] . '/' . $temp['folder']); // удаляем папку раздела галереи
			rmdir($work->config['site_dir'] . $work->config['thumbnail_folder'] . '/' . $temp['folder']); // удаляем папку эскизов раздела

			$db->query("DELETE FROM `category` WHERE `id` = " . $cat); // удаляем запись о разделе

			$redirect_url = $work->config['site_url']; // указываем о необходимости редиректа на главную страницу сайта
		...
		{
			$redirect_url = $work->config['site_url']; // перейдем на основную страницу сайта редиректом
			$redirect_time = 5; // установим время редиректа 5 сек
...
else // иначе если не указан запрос на вывод определенного раздела, то или выведем список всех разделов или выполним доп-команду, не требующую идентификатора раздела (к примеру - добавить категорию)
{
	...
	elseif (isset($_REQUEST['subact']) && $_REQUEST['subact'] == 'saveadd' && $user->user['cat_moderate'] == true) // иначе если поступила под-команда о необходимости сохранить созданный раздел и пользователь имеет право создавать раздел, то...
	{
		...
		if($db->num_rows("SELECT * FROM `category` WHERE `folder` = '" . $name_dir . "'") || is_dir($work->config['site_dir'] . $work->config['gallery_folder'] . '/' . $name_dir) || is_dir($work->config['site_dir'] . $work->config['thumbnail_folder'] . '/' . $name_dir)) // проверяем, если такая папка раздела уже существует, то...
		{
		...
		if(mkdir($work->config['site_dir'] . $work->config['gallery_folder'] . '/' . $name_dir, 0777) && mkdir($work->config['site_dir'] . $work->config['thumbnail_folder'] . '/' . $name_dir, 0777)) // если удалось создать папки для хранения изображений и эскизов раздела, то..
		{
			@copy($work->config['site_dir'] . $work->config['gallery_folder'] . '/index.php', $work->config['site_dir'] . $work->config['gallery_folder'] . '/' . $name_dir . '/index.php'); // скопируем индексный файл из корневой папки раздела (позволить скрыть вывод списка изображений)
			@copy($work->config['site_dir'] . $work->config['thumbnail_folder'] . '/index.php', $work->config['site_dir'] . $work->config['thumbnail_folder'] . '/' . $name_dir . '/index.php'); // и скопируем индексный файл из корневой папки эскизов раздела (позволить скрыть вывод списка эскизов изображений)
			...
			{
				$redirect_url = $work->config['site_url'] . '?action=category&cat=' . $new_cat; // организуем редирект в указанный раздел
			}
			else
			{
				$redirect_url = $work->config['site_url'] . '?action=category'; // иначе редирект будет к списку категорий
			}
			$redirect_time = 5; // установим время редиректа 5 сек
		...
		{
			$redirect_url = $work->config['site_url'] . '?action=category&subact=add'; // сделаем редирект обратно на форму добавления раздела
			$redirect_time = 5; // установим время редиректа 5 сек
