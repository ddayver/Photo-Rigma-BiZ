...
if (!$work->check_get('id', true, true, '^[0-9]+\$') || $user->user['pic_view'] != true) {
    $photo_id = 0;
} else {
    $photo_id = $_GET['id'];
}
...
if ($db->select(array('file', 'category'), TBL_PHOTO, '`id` = ' . $photo_id)) {
    ...
    if ($temp) {
        if ($db->select('folder', TBL_CATEGORY, '`id` = ' . $temp['category'])) {
            $temp2 = $db->res_row();
            if ($temp2) {
                if (!@fopen($work->config['site_dir'] . $work->config['gallery_folder'] . '/' . $temp2['folder'] . '/' . $temp['file'], 'r')) {
                    $photo_id = 0;
                }
            } else {
                $photo_id = 0;
...
if ($photo_id != 0) {
                    ...
                    if ($work->check_get('subact', true, true) && $_GET['subact'] == "rate" && ($user->user['pic_rate_user'] == true || $user->user['pic_rate_moder'] == true) && $temp_photo['user_upload'] != $user->user['id']) {
                        ...
                        if ($user->user['pic_rate_moder'] == true && $work->check_post('rate_moder', true, true, '^\-?[0-9]+\$') && abs($_POST['rate_moder']) <= $work->config['max_rate'] && $rate_moder === false) {
                            ...
                            if ($db->select('rate', TBL_RATE_MODER, '`id_foto` = ' . $photo_id)) {
                                ...
                                if (!$db->update(array('rate_moder' => $rate_moder), TBL_PHOTO, '`id` = ' . $photo_id)) {
                                    log_in_file($db->error, DIE_IF_ERROR);
                                }
                            } else {
                                log_in_file($db->error, DIE_IF_ERROR);
                        ...
                        if ($db->select('*', TBL_PHOTO, '`id` = ' . $photo_id)) {
                            ...
                            if ($temp_photo) {
                                ...
                                if (!$work->check_post('name_photo', true, true)) {
                                    ...
                                    $photo['name'] = trim(htmlentities($_POST['name_photo']));
                                }

                                if (!$work->check_post('description_photo', true, true)) {
                                    $photo['description'] = $temp_photo['description'];
                                } else {
                                    $photo['description'] = trim(htmlentities($_POST['description_photo']));
                                }
                                ...
                                if (!$work->check_post('name_category', true, true, '^[0-9]+\$')) {
                                    ...
                                    if ($user->user['cat_user'] == true || $user->user['pic_moderate'] == true) {
                                ...
                                if (!$db->update(array('name' => $photo['name'], 'description' => $photo['description'], 'category' => $photo['category_name']), TBL_PHOTO, '`id` = ' . $photo_id)) {
                                    log_in_file($db->error, DIE_IF_ERROR);
                                }
                            } else {
    ...
    if ($work->check_get('subact', true, true) && $_GET['subact'] == "upload" && $user->user['id'] != 0 && $user->user['pic_upload'] == true) {
        ...
        if (!$work->check_post('description_photo', true, true)) {
            $photo['description'] = $lang['photo']['no_description'] . ' (' . $work->encodename(basename($_FILES['file_photo']['name'])) . ')';
        } else {
