...
if (!$user->user['pic_view'] || !$work->check_get('id', true, true, '/^[0-9]+$/')) {
    $photo_id = 0;
} else {
    $photo_id = $_GET['id'];
}
...
if ($db->select(array('file', 'category'), TBL_PHOTO, '`id` = ' . $photo_id)) {
    ...
    if ($temp) {
        if ($db->select('folder', TBL_CATEGORY, '`id` = ' . $temp['category'])) {
            ...
            if ($temp2) {
                if (!@fopen(
                    $work->config['site_dir'] . $work->config['gallery_folder'] . '/' . $temp2['folder'] . '/' . $temp['file'],
                    'r'
                )) {
                    $photo_id = 0;
                }
...
if ($photo_id !== 0) {
    if ($db->select('*', TBL_PHOTO, '`id` = ' . $photo_id)) {
        ...
        if ($temp_photo) {
            if ($db->select('*', TBL_CATEGORY, '`id` = ' . $temp_photo['category'])) {
                ...
                if ($temp_category) {
                    if (($user->user['pic_rate_user'] || $user->user['pic_rate_moder']) && $temp_photo['user_upload'] !== $user->user['id'] && $work->check_get(
                        'subact',
                        true,
                        true
                    ) && $_GET['subact'] === "rate") {
                        ...
                        if ($user->user['pic_rate_user'] && $rate_user === false && $work->check_post(
                            'rate_user',
                            true,
                            true,
                            '/^-?[0-9]+$/'
                            ...
                            if (!$db->insert($query_rate, TBL_RATE_USER, 'ignore')) {
                                log_in_file($db->error, DIE_IF_ERROR);
                            }
                            if ($db->select('rate', TBL_RATE_USER, '`id_foto` = ' . $photo_id)) {
                                ...
                                $rate_user = 0;
                                if ($temp) {
                                    $rate_user = array_sum(array_column($temp ?? [], 'rate')) / count($temp);
                                }
                                if (!$db->update(array('rate_user' => $rate_user), TBL_PHOTO, '`id` = ' . $photo_id)) {
                                    log_in_file($db->error, DIE_IF_ERROR);
                                }
                            } else {
                                log_in_file($db->error, DIE_IF_ERROR);
                        ...
                        if ($user->user['pic_rate_moder'] && $rate_moder === false && $work->check_post(
                            'rate_moder',
                            true,
                            true,
                            '^\-?[0-9]+\$'
                            ...
                            $query_rate['id_user'] = $user->user['id'];
                            $query_rate['rate'] = $_POST['rate_user'];
                            if (!$db->insert($query_rate, TBL_RATE_MODER, 'ignore')) {
                                log_in_file($db->error, DIE_IF_ERROR);
                            }
                            if ($db->select('rate', TBL_RATE_MODER, '`id_foto` = ' . $photo_id)) {
                                ...
                                $rate_moder = 0;
                                if ($temp) {
                                    $rate_moder = array_sum(array_column($temp ?? [], 'rate')) / count($temp);
                                }
                                if (!$db->update(
                                    array('rate_moder' => $rate_moder),
                                    TBL_PHOTO,
                                    '`id` = ' . $photo_id
                                )) {
                                    log_in_file($db->error, DIE_IF_ERROR);
                        ...
                        true,
                        true
                    ) && $_GET['subact'] === 'saveedit' && (($temp_photo['user_upload'] === $user->user['id'] && $user->user['id'] !== 0) || (bool)$user->user['pic_moderate'])) {
                        if ($db->select('*', TBL_PHOTO, '`id` = ' . $photo_id)) {
                            $temp_photo = $db->res_row();
                            if ($temp_photo) {
                                ...
                                if (!$work->check_post('name_category', true, true, '^[0-9]+\$')) {
                                    ...
                                    if ((bool)$user->user['cat_user'] || (bool)$user->user['pic_moderate']) {
                    ...
                    if ($work->check_get(
                        ...
                        true,
                        true
                    ) && $_GET['subact'] === "edit" && (($temp_photo['user_upload'] === $user->user['id'] && $user->user['id'] !== 0) || (bool)$user->user['pic_moderate'])) {
                                    ...
                                    if ($temp_category) {
                                        ...
                                        if ($db->select('*', TBL_CATEGORY, $select_cat)) {
                                            ...
                                            if ($temp_category) {
                                                ...
                                                $template->add_string_ar(array(
                                                    'L_NAME_BLOCK' => $work->lang['photo']['edit'] . ' - ' . $temp_photo['name'],
                        ...
                        true,
                        true
                    ) && $_GET['subact'] === "delete" && (($temp_photo['user_upload'] === $user->user['id'] && $user->user['id'] !== 0) || (bool)$user->user['pic_moderate'])) {
                        $photo['name'] = $temp_photo['name'];
                        if ($temp_category['id'] === 0) {
                            if ($db->select(
                                'COUNT(*) as `count_photo`',
                                TBL_PHOTO,
                                '`id` != ' . $photo_id . ' AND `category` = 0 AND `user_upload` = ' . $temp_photo['user_upload']
                            )) {
                                $temp = $db->res_row();
                                if (isset($temp['count_photo']) && $temp['count_photo'] > 0) {
                                    $photo['category_url'] = $work->config['site_url'] . '?action=category&cat=user&id=' . $temp_photo['user_upload'];
                                } else {
                                    if ($db->select(
                                        'COUNT(*) as `count_photo`',
                                        TBL_PHOTO,
                                        ...
                                        $temp = $db->res_row();
                                        if (isset($temp['count_photo']) && $temp['count_photo'] > 0) {
                                            $photo['category_url'] = $work->config['site_url'] . '?action=categorycat=user';
                                        } else {
                                            if ($db->select(
                                                'COUNT(*) as `count_photo`',
                                                TBL_PHOTO,
                            ...
                            if ($db->select(
                                'COUNT(*) as `count_photo`',
                                TBL_PHOTO,
                                '`id` != ' . $photo_id . ' AND `category` = ' . $temp_category['id']
                            )) {
                                $temp = $db->res_row();
                                if (isset($temp['count_photo']) && $temp['count_photo'] > 0) {
                            ...
                            if ($temp_photo) {
                                if ($db->select('*', TBL_CATEGORY, '`id` = ' . $temp_photo['category'])) {
                                    ...
                                    if ($temp_category) {
                                        ...
                                        $template->add_string_ar(array(
                                            'L_NAME_BLOCK' => $work->lang['photo']['title'] . ' - ' . $temp_photo['name'],
                                        ...
                                        if (($temp_photo['user_upload'] === $user->user['id'] && $user->user['id'] !== 0) || (bool)$user->user['pic_moderate']) {
                                            $template->add_string_ar(array(
                                                'L_EDIT_BLOCK' => $work->lang['photo']['edit'],
                                                'L_CONFIRM_DELETE_BLOCK' => $work->lang['photo']['confirm_delete'] . ' ' . $temp_photo['name'],
                                        ...
                                        $template->add_string_ar(array(
                                            'D_RATE_USER' => $work->lang['photo']['rate_user'] . ': ' . $temp_photo['rate_user'],
                                            'D_RATE_MODER' => $work->lang['photo']['rate_moder'] . ': ' . $temp_photo['rate_moder']
                                        ));
                                        if ((bool)$user->user['pic_rate_user']) {
                                            if ($db->select(
                                                'rate',
                                                TBL_RATE_USER,
                                                '`id_foto` = ' . $photo_id . ' AND `id_user` = ' . $user->user['id']
                                        ...
                                        if ((bool)$user->user['pic_rate_moder']) {
                                            if ($db->select(
                                                'rate',
                                                TBL_RATE_MODER,
                                                '`id_foto` = ' . $photo_id . ' AND `id_user` = ' . $user->user['id']
                                        ...
                                        if ((((bool)$user->user['pic_rate_user'] || (bool)$user->user['pic_rate_moder']) && $temp_photo['user_upload'] !== $user->user['id']) && ($user_rate !== false || $moder_rate !== false)) {
    ...
    if ($work->check_get(
        ...
        true,
        true
    ) && $_GET['subact'] === "upload" && $user->user['id'] !== 0 && (bool)$user->user['pic_upload']) {
        ...
        true,
        true
    ) && $_GET['subact'] === "uploaded" && $user->user['id'] !== 0 && (bool)$user->user['pic_upload']) {
        $submit_upload = true;
        $max_size_php = Work::return_bytes(ini_get('post_max_size'));
        $max_size = Work::return_bytes($work->config['max_file_size']);
        if ($max_size > $max_size_php) {
...
if ((isset($_GET['subact']) && $_GET['subact'] === "uploaded" && $user->user['id'] !== 0 && (bool)$user->user['pic_upload'])) {
