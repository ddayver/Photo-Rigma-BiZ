...
if ((!$work->check_session('admin_on', TRUE) || $_SESSION['admin_on'] !== TRUE) && $user->user['admin'] == TRUE && $work->check_post('admin_password', TRUE, TRUE) && $user->user['password'] == md5($_POST['admin_password'])) // если не открыта сессия для Админа, пользователь имеет право на вход в Админку, был передан пароль для входа и пароль совпадает с паролем пользователя, то...
{
	$_SESSION['admin_on'] = TRUE; // открываем сессию для Админа
}

if ($work->check_session('admin_on', TRUE) && $_SESSION['admin_on'] === TRUE && $user->user['admin'] == TRUE) // если открыта сессия для Админа и пользователь имеет права Админа, то...
{
	if ($work->check_get('subact', TRUE, TRUE) && $_GET['subact'] == 'settings') // если была команда на общие настройки, то...
	{
		if ($work->check_post('submit', TRUE, TRUE) || ($work->check_post('submit_x', TRUE, TRUE) && $work->check_post('submit_y', TRUE, TRUE))) // если поступил запрос на сохранение общих настроек, то...
		{
			...
			if ($work->check_post('title_name', TRUE, TRUE, REG_NAME)) $new_config['title_name'] = $work->clean_field($_POST['title_name']);
			if ($work->check_post('title_description', TRUE, TRUE, REG_NAME)) $new_config['title_description'] = $work->clean_field($_POST['title_description']);
			if ($work->check_post('meta_description', TRUE, TRUE, REG_NAME)) $new_config['meta_description'] = $work->clean_field($_POST['meta_description']);
			if ($work->check_post('meta_keywords', TRUE, TRUE, REG_NAME)) $new_config['meta_keywords'] = $work->clean_field($_POST['meta_keywords']);
			if ($work->check_post('gal_width', TRUE, TRUE, '^([0-9]+)(%){0,1}\$')) $new_config['gal_width'] = $_POST['gal_width'];
			if ($work->check_post('left_panel', TRUE, TRUE, '^([0-9]+)(%){0,1}\$')) $new_config['left_panel'] = $_POST['left_panel'];
			if ($work->check_post('right_panel', TRUE, TRUE, '^([0-9]+)(%){0,1}\$')) $new_config['right_panel'] = $_POST['right_panel'];
			if ($work->check_post('language', TRUE, TRUE, REG_LOGIN) && is_dir($work->config['site_dir'] . 'language/' . $_POST['language'])) $new_config['language'] = $_POST['language'];
			if ($work->check_post('themes', TRUE, TRUE, REG_LOGIN) && is_dir($work->config['site_dir'] . 'themes/' . $_POST['themes'])) $new_config['themes'] = $_POST['themes'];
			if ($work->check_post('max_file_size', TRUE, TRUE, '^[0-9]+\$') && $work->check_post('max_file_size_letter', TRUE, TRUE, '^(B|K|M|G)\$'))
			{
			...
			if ($work->check_post('max_photo_w', TRUE, TRUE, '^[0-9]+\$')) $new_config['max_photo_w'] = $_POST['max_photo_w'];
			if ($work->check_post('max_photo_h', TRUE, TRUE, '^[0-9]+\$')) $new_config['max_photo_h'] = $_POST['max_photo_h'];
			if ($work->check_post('temp_photo_w', TRUE, TRUE, '^[0-9]+\$')) $new_config['temp_photo_w'] = $_POST['temp_photo_w'];
			if ($work->check_post('temp_photo_h', TRUE, TRUE, '^[0-9]+\$')) $new_config['temp_photo_h'] = $_POST['temp_photo_h'];
			if ($work->check_post('max_avatar_w', TRUE, TRUE, '^[0-9]+\$')) $new_config['max_avatar_w'] = $_POST['max_avatar_w'];
			if ($work->check_post('max_avatar_h', TRUE, TRUE, '^[0-9]+\$')) $new_config['max_avatar_h'] = $_POST['max_avatar_h'];
			if ($work->check_post('copyright_year', TRUE, TRUE, '^[0-9\-]+\$')) $new_config['copyright_year'] = $_POST['copyright_year'];
			if ($work->check_post('copyright_text', TRUE, TRUE, REG_NAME)) $new_config['copyright_text'] = $work->clean_field($_POST['copyright_text']);
			if ($work->check_post('copyright_url', TRUE, TRUE)) $new_config['copyright_url'] = $_POST['copyright_url'];
			if ($work->check_post('last_news', TRUE, TRUE, '^[0-9]+\$')) $new_config['last_news'] = $_POST['last_news'];
			if ($work->check_post('best_user', TRUE, TRUE, '^[0-9]+\$')) $new_config['best_user'] = $_POST['best_user'];
			if ($work->check_post('max_rate', TRUE, TRUE, '^[0-9]+\$')) $new_config['max_rate'] = $_POST['max_rate'];
		...
		foreach ($language as $key => $val) $template->add_string_ar(array(
				'D_DIR_LANG'      => $val['value'],
				'D_NAME_LANG'     => $val['name'],
				'D_SELECTED_LANG' => $val['value'] == $work->config['language'] ? ' selected="selected"' : ''
			), 'SELECT_LANGUAGE[' . $key . ']');
		foreach ($themes as $key => $val) $template->add_string_ar(array(
				'D_DIR_THEMES'      => $val,
	...
	elseif ($work->check_get('subact', TRUE, TRUE) && $_GET['subact'] == 'admin_user') // иначе если было запрошено Управление пользователями
	{
		$template->add_case('ADMIN_BLOCK', 'ADMIN_USER');
		$template->add_string('L_NAME_BLOCK', $lang['admin']['title'] . ' - ' . $lang['admin']['admin_user']);

		if ($work->check_get('uid', TRUE, TRUE, '^[0-9]+\$', TRUE)) // если был передан идентификатор пользователя для редактирования, то...
		{
			if ($db->select('*', TBL_USERS, '`id` = ' . $work->clean_field($_GET['uid'])))
			{
				...
				if ($temp)
				{
					if ($work->check_post('submit', TRUE, TRUE))
					{
						if ($work->check_post('group', TRUE, TRUE, '^[0-9]+\$', TRUE) && $_POST['group'] != $temp['group'])
						{
							...
							if ($db->select('*', TBL_GROUP, '`id` = ' . $work->clean_field($_POST['group'])))
							{
								...
								if ($temp_group)
								{
									...
									if ($db->update($query, TBL_USERS, '`id` = ' . $work->clean_field($_GET['uid']))) $temp = $new_temp;
									else log_in_file($db->error, DIE_IF_ERROR);
								}
						...
						else
						{
							foreach ($temp as $key => $value)
							{
								...
								{
									if ($work->check_post($key, TRUE) && ($_POST[$key] == 'on' || $_POST[$key] === TRUE)) $_POST[$key] = '1';
									else $_POST[$key] = '0';
									if ($_POST[$key] != $value)
									{
										if ($db->update(array($key => $_POST[$key]), TBL_USERS, '`id` = ' . $work->clean_field($_GET['uid']))) $temp[$key] = $_POST[$key];
										else log_in_file($db->error, DIE_IF_ERROR);
									}
								}
							}
					...
					if ($db->select('*', TBL_GROUP, '`id` !=0'))
					{
						...
						if ($group)
						{
							foreach ($group as $key => $val) $template->add_string_ar(array(
									'D_GROUP_ID'       => $val['id'],
							...
							foreach ($temp as $key => $value)
							{
								if ($key != 'id' && $key != 'login' && $key != 'password' && $key != 'real_name' && $key != 'email' && $key != 'avatar' && $key != 'date_regist' && $key != 'date_last_activ' && $key != 'date_last_logout' && $key != 'group')
								{
									...
									if ($value == 1 || $value == '1' || $value === TRUE) $template->add_string('D_' . strtoupper($key), ' checked="checked"');
									else $template->add_string('D_' . strtoupper($key), '');
								}
							}
		...
		else
		{
			if ($work->check_post('submit', TRUE, TRUE) && $work->check_post('search_user', TRUE, TRUE))
			{
				...
				if ($db->select('*', TBL_USERS, '`real_name` LIKE \'%' . $work->clean_field($_POST['search_user']) . '%\''))
				{
					$find = $db->res_arr();
					if ($find)
					{
					...
					else $template->add_string_ar(array(
	...
	elseif ($work->check_get('subact', TRUE, TRUE) && $_GET['subact'] == 'admin_group') // иначе если запрошено Управление группами, то...
	{
		...
		if ($work->check_post('submit', TRUE, TRUE) && $work->check_post('id_group', TRUE, FALSE, '^[0-9]+\$'))
		{
			if ($db->select('*', TBL_GROUP, '`id` = ' . $work->clean_field($_POST['id_group'])))
			{
				...
				if ($temp)
				{
					if ($work->check_post('name_group', TRUE, TRUE, REG_NAME) && $_POST['name_group'] != $temp['name'])
					{
						if ($db->update(array('name' => $work->clean_field($_POST['name_group'])), TBL_GROUP, '`id` = ' . $work->clean_field($_POST['id_group']))) $temp['name'] = $work->clean_field($_POST['name_group']);
						else log_in_file($db->error, DIE_IF_ERROR);
					}
					foreach ($temp as $key => $value)
					{
						if ($key != 'id' && $key != 'name')
						{
							if ($work->check_post($key, TRUE, TRUE) && ($_POST[$key] == 'on' || $_POST[$key] === TRUE)) $_POST[$key] = '1';
							else $_POST[$key] = '0';
							if ($_POST[$key] != $value)
							{
								if ($db->update(array($key => $_POST[$key]), TBL_GROUP, '`id` = ' . $work->clean_field($_POST['id_group']))) $temp[$key] = $_POST[$key];
								else log_in_file($db->error, DIE_IF_ERROR);
							}
						}
					}
			...
			if ($db->select('*', TBL_GROUP, '`id` = ' . $work->clean_field($_POST['group'])))
			{
				...
				if ($temp)
				{
					foreach ($temp as $key => $value)
					{
						...
						{
							$template->add_string('L_' . strtoupper($key), $lang['admin'][$key]);
							if ($value == 1) $template->add_string('D_' . strtoupper($key), ' checked="checked"');
							else $template->add_string('D_' . strtoupper($key), '');
						}
					}
