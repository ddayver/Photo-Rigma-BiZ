...
if ($work->check_session('admin_on', true) && $_SESSION['admin_on'] === true && $user->user['admin'] == true) { // если открыта сессия для Админа и пользователь имеет права Админа, то...
    if ($work->check_get('subact', true, true) && $_GET['subact'] == 'settings') { // если была команда на общие настройки, то...
        if ($work->check_post('submit', true, true) || ($work->check_post('submit_x', true, true) && $work->check_post('submit_y', true, true))) { // если поступил запрос на сохранение общих настроек, то...
            ...
            if ($work->check_post('language', true, true, REG_LOGIN) && is_dir($work->config['site_dir'] . 'language/' . $_POST['language'])) {
                $new_config['language'] = $_POST['language'];
            }
            if ($work->check_post('themes', true, true, REG_LOGIN) && is_dir($work->config['site_dir'] . 'themes/' . $_POST['themes'])) {
                        ...
                        if ($work->check_post('group', true, true, '^[0-9]+\$', true) && $_POST['group'] != $temp['group']) {
                            ...
                            foreach ($temp as $key => $value) {
                                if ($key != 'id' && $key != 'login' && $key != 'password' && $key != 'real_name' && $key != 'email' && $key != 'avatar' && $key != 'date_regist' && $key != 'date_last_activ' && $key != 'date_last_logout' && $key != 'group') {
                                    if ($work->check_post($key, true) && ($_POST[$key] == 'on' || $_POST[$key] === true)) {
                                        $_POST[$key] = '1';
                                    } else {
                                        $_POST[$key] = '0';
                                    }
                                    if ($_POST[$key] != $value) {
                                        if ($db->update(array($key => $_POST[$key]), TBL_USERS, '`id` = ' . $work->clean_field($_GET['uid']))) {
                                            $temp[$key] = $_POST[$key];
                                        } else {
                                            log_in_file($db->error, DIE_IF_ERROR);
                                        }
            ...
            if ($db->select('*', TBL_GROUP, '`id` = ' . $work->clean_field($_POST['id_group']))) {
                ...
                if ($temp) {
                    if ($work->check_post('name_group', true, true, REG_NAME) && $_POST['name_group'] != $temp['name']) {
                        if ($db->update(array('name' => $work->clean_field($_POST['name_group'])), TBL_GROUP, '`id` = ' . $work->clean_field($_POST['id_group']))) {
                    ...
                    foreach ($temp as $key => $value) {
                        if ($key != 'id' && $key != 'name') {
                            if ($work->check_post($key, true, true) && ($_POST[$key] == 'on' || $_POST[$key] === true)) {
                                $_POST[$key] = '1';
                            } else {
                                $_POST[$key] = '0';
                            }
                            if ($_POST[$key] != $value) {
                                if ($db->update(array($key => $_POST[$key]), TBL_GROUP, '`id` = ' . $work->clean_field($_POST['id_group']))) {
                                    $temp[$key] = $_POST[$key];
                                } else {
                                    log_in_file($db->error, DIE_IF_ERROR);
                                }
