...
/// @cond
if (!$work->check_get('foto', true, true, '^[0-9]+\$', true) || $user->user['pic_view'] == false) {
    $temp_photo = $work->no_photo();
} else {
...
if (!@fopen($temp_photo['full_path'], 'r')) {
    $temp_photo = $work->no_photo();
}

if ($work->check_get('thumbnail', true, true) && $_GET['thumbnail'] == 1) {
    if ($work->image_resize($temp_photo['full_path'], $temp_photo['thumbnail_path'])) {
        echo $work->image_attach($temp_photo['thumbnail_path'], $temp_photo['file']);
    } else {
        log_in_file('Error Image Resize: ' . $temp_photo['full_path'], DIE_IF_ERROR);
    }
} else {
    echo $work->image_attach($temp_photo['full_path'], $temp_photo['file']);
}
/// @endcond
