...
interface Database_Interface
{
     ...
     *
     * @details Этот метод является частью контракта, который должны реализовать классы, использующие интерфейс.
     *          Метод предназначен для начала транзакции в базе данных через объект PDO.
     *          После начала транзакции рекомендуется записать лог с указанием контекста.
     *          Реализация должна принимать параметры напрямую без дополнительной обработки.
     *
     * @callgraph
     *
     ...
     *
     * @note    Реализация метода должна использовать логирование для записи информации о начале транзакции.
     * @warning Убедитесь, что соединение с базой данных установлено перед вызовом метода.
     *
     * Пример вызова метода:
     * @code
     * $object = new \PhotoRigma\Classes\Database();
     * $object->begin_transaction('Сохранение данных пользователя');
     * // Лог: [DB] Транзакция начата | Контекст: Сохранение данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::begin_transaction() Реализация метода в классе Database.
     *
     */
     ...
     *
     * @details Этот метод является частью контракта, который должны реализовать классы, использующие интерфейс.
     *          Метод предназначен для подтверждения транзакции в базе данных через объект PDO.
     *          После подтверждения транзакции рекомендуется записать лог с указанием контекста.
     *          Реализация должна принимать параметры напрямую без дополнительной обработки.
     *
     * @callgraph
     *
     ...
     *
     * @note    Реализация метода должна использовать логирование для записи информации о подтверждении транзакции.
     * @warning Убедитесь, что транзакция была начата перед вызовом этого метода.
     *
     * Пример вызова метода:
     * @code
     * $object = new \PhotoRigma\Classes\Database();
     * $object->commit_transaction('Сохранение данных пользователя');
     * // Лог: [DB] Транзакция подтверждена | Контекст: Сохранение данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::commit_transaction() Реализация метода в классе Database.
     *
     */
     ...
     *
     * @details Этот метод является частью контракта, который должны реализовать классы, использующие интерфейс.
     *          Метод предназначен для отмены транзакции в базе данных через объект PDO.
     *          После отмены транзакции рекомендуется записать лог с указанием контекста.
     *          Реализация должна принимать параметры напрямую без дополнительной обработки.
     *
     * @callgraph
     *
     ...
     *
     * @note    Реализация метода должна использовать логирование для записи информации об отмене транзакции.
     * @warning Убедитесь, что транзакция была начата перед вызовом этого метода.
     *
     * Пример вызова метода:
     * @code
     * $object = new \PhotoRigma\Classes\Database();
     * $object->rollback_transaction('Отмена сохранения данных пользователя');
     * // Лог: [DB] Транзакция отменена | Контекст: Отмена сохранения данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::rollback_transaction() Реализация метода в классе Database.
     *
     */
...
class Database implements Database_Interface
{
     ...
     * @brief   Начинает транзакцию в базе данных.
     *
     * @details Этот метод является публичным редиректом, вызывающим защищённый метод `_begin_transaction_internal()`.
     *          Метод начинает транзакцию в базе данных через объект PDO.
     *          После начала транзакции записывается лог с указанием контекста.
     *          Параметры передаются напрямую в защищённый метод без дополнительной обработки.
     *          Обратите внимание, что этот метод не содержит собственной логики и полностью полагается на
     ...
     *                        По умолчанию: пустая строка.
     *
     * @throws Exception Выбрасывается, если произошла ошибка при начале транзакции (см.
     *                   `_begin_transaction_internal()`).
     *
     ...
     * // Лог: [DB] Транзакция начата | Контекст: Сохранение данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::_begin_transaction_internal() Защищённый метод, реализующий основную
     *          логику.
     *
     */
    public function begin_transaction(string $context = ''): void
     ...
     * @brief   Начинает транзакцию в базе данных.
     *
     * @details Этот метод начинает транзакцию в базе данных через объект PDO.
     *          После начала транзакции записывается лог с указанием контекста.
     ...
     * // Лог: [DB] Транзакция начата | Контекст: Сохранение данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::begin_transaction() Публичный метод-редирект для вызова этой логики.
     *
     ...
     * @brief   Подтверждает транзакцию в базе данных.
     *
     * @details Этот метод является публичным редиректом, вызывающим защищённый метод `_commit_transaction_internal()`.
     *          Метод подтверждает транзакцию в базе данных через объект PDO.
     *          После подтверждения транзакции записывается лог с указанием контекста.
     *          Параметры передаются напрямую в защищённый метод без дополнительной обработки.
     *          Обратите внимание, что этот метод не содержит собственной логики и полностью полагается на
     ...
     *                        По умолчанию: пустая строка.
     *
     * @throws Exception Выбрасывается, если произошла ошибка при подтверждении транзакции (см.
     *                   `_commit_transaction_internal()`).
     *
     ...
     * // Лог: [DB] Транзакция подтверждена | Контекст: Сохранение данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::_commit_transaction_internal() Защищённый метод, реализующий основную
     *          логику.
     *
     */
    public function commit_transaction(string $context = ''): void
     ...
     * @brief   Подтверждает транзакцию в базе данных.
     *
     * @details Этот метод подтверждает транзакцию в базе данных через объект PDO.
     *          После подтверждения транзакции записывается лог с указанием контекста.
     *          Этот метод является защищенным и предназначен для использования внутри класса или его наследников.
     ...
     * // Лог: [DB] Транзакция подтверждена | Контекст: Сохранение данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::commit_transaction() Публичный метод-редирект для вызова этой логики.
     *
     * @see     PhotoRigma::Classes::Database::$pdo Объект PDO для подключения к базе данных.
     * @see     PhotoRigma::Include::log_in_file() Логирует сообщения в файл.
     */
     ...
     * @brief   Отменяет транзакцию в базе данных.
     *
     * @details Этот метод является публичным редиректом, вызывающим защищённый метод
     *          `_rollback_transaction_internal()`. Метод отменяет транзакцию в базе данных через объект PDO. После
     *          отмены транзакции записывается лог с указанием контекста. Параметры передаются напрямую в защищённый
     *          метод без дополнительной обработки. Обратите внимание, что этот метод не содержит собственной логики и
     *          полностью полагается на `_rollback_transaction_internal()`.
     *
     * @callgraph
     *
     * @param string $context Контекст транзакции (необязательный параметр).
     ...
     * // Лог: [DB] Транзакция отменена | Контекст: Отмена сохранения данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::_rollback_transaction_internal() Защищённый метод, реализующий основную
     *          логику.
     *
     */
    public function rollback_transaction(string $context = ''): void
     ...
     * @brief   Отменяет транзакцию в базе данных.
     *
     * @details Этот метод отменяет транзакцию в базе данных через объект PDO.
     *          После отмены транзакции записывается лог с указанием контекста.
     *          Этот метод является защищенным и предназначен для использования внутри класса или его наследников.
     *          Основная логика метода вызывается через публичный метод-редирект `rollback_transaction()`.
     *
     * @callergraph
     * @callgraph
     *
     ...
     * // Лог: [DB] Транзакция отменена | Контекст: Отмена сохранения данных пользователя
     * @endcode
     * @see     PhotoRigma::Classes::Database::rollback_transaction() Публичный метод-редирект для вызова этой логики.
     *
     * @see     PhotoRigma::Classes::Database::$pdo Объект PDO для подключения к базе данных.
     * @see     PhotoRigma::Include::log_in_file() Логирует сообщения в файл.
     */
