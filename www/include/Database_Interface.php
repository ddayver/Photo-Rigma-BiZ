 ...
 *              данных, поддерживающими SQL (MySQL/MariaDB, PostgreSQL и SQLite). Интерфейс обеспечивает единообразие
 *              реализации различных СУБД и предоставляет контракт для выполнения основных операций:
 ...
 * @brief     Интерфейс для работы с базами данных через PDO.
 *
 * @details   Этот интерфейс определяет контракт для работы с базами данных, поддерживающими SQL (MySQL/MariaDB,
 *            PostgreSQL и SQLite). Он предоставляет методы для выполнения основных операций с базой данных:
 *            - Выборка данных (`SELECT`, `JOIN`).
 *            - Вставка данных (`INSERT`, `REPLACE`, `INSERT IGNORE`).
 *            - Обновление данных (`UPDATE`).
 *            - Удаление данных (`DELETE`, `TRUNCATE`).
 ...
 *            - Форматирование даты для SQL-запросов.
 *            - Управление транзакциями (начало, подтверждение, откат).
 *            - Временное изменение формата SQL для выполнения запросов в контексте другой СУБД (например, выполнение
...
interface Database_Interface
{
     ...
     *          2. Сохраняет текущий формат SQL в свойстве объекта.
     ...
     *        Может быть целым числом (например, 10), строкой (например, "0,10") или false (без ограничения).
     *
