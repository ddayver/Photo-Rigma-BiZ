 ...
 *              также используется для хранения всех данных о текущем пользователе. Остальное будет добавлено по мере
 ...
 *
 * @details   Интерфейс определяет контракт для классов, реализующих методы добавления, обновления, удаления
...
interface User_Interface
{
     ...
     *        Метод является частью контракта интерфейса для управления данными пользователя и сессии при
     ...
     * $userId = $user->add_new_user($_POST, $redirectUrl);
     * if ($userId > 0) {
     ...
     *
     */
    public function add_new_user(array $post_data, string $redirect_url): int;
     ...
     *                          - 're_password' (string): Повторный ввод нового пароля (должен совпадать с
     *                          'edit_password').
     *                          - 'email' (string): Новый email пользователя (должен соответствовать регулярному
     *                          выражению REG_EMAIL).
     ...
     *
     * @throws RuntimeException Выбрасывается исключение если пользователь с указанным `$user_id` не найден в базе
     *                          данных.
     *
     ...
     *          - Проверять, изменилась ли группа пользователя в `$post_data`. Если группа изменилась:
     *            - Получать данные новой группы из таблицы групп.
     *            - Обновлять ID группы пользователя и связанные права доступа.
     *            - Декодировать обновленные права доступа пользователя в массив.
     *          - Если группа не изменилась:
     *            - Проверять, отличаются ли текущие права пользователя от переданных в `$post_data`.
     *            - Обновлять права доступа пользователя, кодируя их в JSON-строку.
     *          - Возвращать обновленные данные пользователя, включая все поля прав доступа и ID группы.
     ...
     *
     * @throws InvalidArgumentException Выбрасывается, если входные данные некорректны (например, отрицательный
     *                                  `user_id` или невалидный `post_data`).
     *
     ...
     *
     * @throws InvalidArgumentException Выбрасывается, если входные данные некорректны (например, отсутствует имя
     *                                  группы или невалидный JSON).
     *
     * @warning Убедитесь, что входные данные корректны перед вызовом метода.
     *
     * Пример вызова метода:
     ...
     * @details Этот метод является частью контракта, который должны реализовать классы, использующие интерфейс.
     *          Метод должен выполнять следующие действия:
     *          - Проверять и обновлять название группы, если оно изменилось в `$post_data`.
     *          - Обновлять права доступа группы:
     *            - Проверять каждое поле прав доступа из свойства класса `user_right_fields`.
     *            - Нормализовать значения прав доступа (true/false).
     *            - Кодировать обновленные права доступа в JSON-строку для хранения в БД.
     *          - Обновлять поле `user_rights` в таблице групп.
     *          - Возвращать обновленные данные группы, включая название и все поля прав доступа.
     *          Метод предназначен для прямого использования извне (например, через Админку).
     *
     ...
     *               - Все поля, указанные в свойстве класса `user_right_fields`.
     *
     * @throws InvalidArgumentException Выбрасывается, если входные данные некорректны (например, отсутствует
     *                                  `id_group` или невалидный `user_rights`).
     *
     ...
     *          - Возвращать результат операции: `true`, если группа успешно удалена, и `false`, если удаление не
     *          выполнено. Метод предназначен для прямого использования извне (например, через Админку).
     *
     ...
     *        Метод является частью контракта интерфейса для обеспечения безопасности форм при взаимодействии с
     *        пользователями.
     *
     * @details Этот метод выполняет следующие действия:
     *          - Если CSRF-токен отсутствует в сессии, он генерируется с использованием `random_bytes(32)` (64
     *          символа).
     ...
     * @callgraph
     *
     * @return string CSRF-токен длиной 64 символа, сгенерированный с использованием безопасного источника случайных
     *                данных.
     *
     ...
     *                            Если значение не является строкой или содержит невалидный JSON, выбрасывается
     *                            исключение.
     *
     ...
     * @warning Убедитесь, что поле `user_rights` содержит валидный JSON. Невалидные данные могут привести к
     *          исключению.
     *
     * Пример вызова метода:
     * @code
...
class User implements User_Interface
{
     ...
     * $user = new \PhotoRigma\Classes\User($db, $_SESSION);
     * @endcode
     * @see     PhotoRigma::Classes::User::all_right_fields() Наполнение массива именами полей прав пользователей и
     *          групп.
     *
    ...
    /**
     * @brief   Метод загружает права первого пользователя и первой группы из базы данных, извлекает имена полей
     *          (ключи)
     ...
     * $this->all_right_fields();
     * @endcode
     * @see     PhotoRigma::Classes::User::$user_right_fields Свойство, содержащее имена полей прав пользователей и
     *          групп.
     *
     * @see     PhotoRigma::Classes::User::$db Свойство, содержащее объект для работы с базой данных.
     ...
     *          3. Декодирует строку JSON в ассоциативный массив. Если декодирование завершается с ошибкой,
     ...
     * @see     PhotoRigma::Classes::User::load_authenticated_user() Метод для загрузки данных аутентифицированного
     *          пользователя.
     *
     * Пример вызова метода внутри класса:
     * @code
     ...
     * @throws UnexpectedValueException|JsonException Если данные группы гостя некорректны (например, пустые или не
     *                                                являются массивом).
     *
     ...
     *          группы, обновляет дату последней активности и сохраняет данные в свойство `$user`.
     *
     * @details Метод выполняет следующие шаги:
     *          1. Загружает данные пользователя из таблицы `TBL_USERS` по `user_id`.
     *          2. Если данные пользователя отсутствуют, сбрасывает сессию до гостя и вызывает метод
     *          `load_guest_user()`.
     *          3. Обрабатывает права пользователя (`user_rights`) с помощью метода `process_user_rights()` и сохраняет
     *          их в свойство `$user`.
     ...
     * @throws RuntimeException|JsonException Если не удаётся обновить дату последней активности пользователя в таблице
     *                                        `TBL_USERS`.
     *
     * @note    Используются константы `TBL_USERS` и `TBL_GROUP` для указания таблиц базы данных.
     * @warning Поля `user_rights` в данных пользователя и группы должны содержать валидный JSON. Невалидные данные
     ...
     *
     * @details Метод проходит по массиву данных группы (`$group_data`) и объединяет их с данными пользователя
     *          (`$user`)
     *          согласно следующей логике:
     *          1. Ключ `name`: Значение сохраняется в `$user['group']`, а текущее значение `$user['group']`
     *          перемещается в `$user['group_id']`.
     ...
     *             - Обновляет поле `name` в таблице групп.
     ...
     *             - Кодирует обновленные права доступа в JSON-строку для хранения в БД.
     *          3. Обновляет поле `user_rights` в таблице групп.
     *          4. Возвращает обновленные данные группы, включая название и все поля прав доступа.
     *          Метод является публичным и предназначен для прямого использования извне (например, через Админку).
     *
     * @callgraph
     *
     * @param array $group_data Данные группы.
     ...
     * print_r($result);
     * @endcode
     * @see     PhotoRigma::Classes::User::process_user_rights() Метод, декодирующий JSON-строку прав пользователя в
     *          массив.
     * @see     PhotoRigma::Classes::User::encode_user_rights() Метод, кодирующий массив прав пользователя в
     *          JSON-строку для хранения в БД.
     *
     * @see     PhotoRigma::Classes::User::$user_right_fields Свойство, содержащее массив с допустимыми полями прав
     *          пользователя.
     */
    public function update_group_data(array $group_data, array $post_data): array
    {
     ...
     * @see     PhotoRigma::Classes::Work::fix_file_extension() Метод для корректировки расширения файла на основе его
     *          MIME-типа.
     ...
     *             - Получает данные новой группы из таблицы групп.
     *             - Обновляет ID группы пользователя и связанные права доступа в БД.
     ...
     *             - Обновляет права доступа пользователя в БД, кодируя их в JSON-строку.
