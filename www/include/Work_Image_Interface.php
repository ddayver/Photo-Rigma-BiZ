...
/**
 * @file        include/Work_Image_Interface.php
 ...
 *
 * @details     Этот файл содержит интерфейс `Work_Image_Interface`, который определяет контракт для классов,
 *              реализующих методы для выполнения операций с изображениями:
 ...
 * @brief     Интерфейс для работы с изображениями.
 *
 * @details   Этот интерфейс определяет контракт для классов, реализующих методы для выполнения операций с
 *            изображениями:
 ...
 *            изображений, управление файловой системой и вывод изображений через HTTP.
 *
 ...
 *            системой,
 *            особенно при обработке пользовательского ввода или больших файлов.
 */
interface Work_Image_Interface
{
    /**
     ...
     * $sizes = $imageHelper->size_image($path);
     ...
     * $success = $imageHelper->image_resize($full_path, $thumbnail_path);
     * if ($success) {
     ...
     * $noPhotoData = $imageHelper->no_photo();
     ...
     * $imageHelper->image_attach('/path/to/image.jpg', 'example.jpg');
     * @endcode
     */
     ...
     * $corrected_path = $imageHelper->fix_file_extension($full_path);
     ...
     * $result = $imageHelper->remove_directory($path);
     * if ($result) {
