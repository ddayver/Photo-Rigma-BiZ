 ...
 *          Этот интерфейс определяет контракт, который должны реализовать классы, отвечающие за базовую логику
 *          приложения.
 *
 * @callergraph
 * @callgraph
...
interface Work_CoreLogic_Interface
{
     ...
     *          4. Для корневой категории (`$cat_id = 0`) подсчет количества уникальных пользователей, загрузивших
     *          фотографии.
     *          5. Формирование результирующего массива с информацией о категории или альбоме, включая название,
     *          описание, количество фотографий, данные о последней и лучшей фотографии, а также ссылки на них.
     *
     *          Этот метод должен быть реализован в классах, использующих интерфейс, и может быть вызван через
     *          публичный метод `category()`.
     *
     * @callgraph
     *
     ...
     * 1. Удаляет файлы из каталогов полноразмерных изображений и эскизов, используя пути, заданные в конфигурации
     ...
     * 4. Логирует ошибки, возникающие при удалении файлов или выполнении запросов к базе данных, с помощью функции
     * `log_in_file()`. Этот метод содержит основную логику, вызываемую через публичный метод del_photo().
     *
     * @callgraph
     *
 ...
 *
 * @details Класс Work_CoreLogic предоставляет реализацию всех методов, определенных в интерфейсе
 *          Work_CoreLogic_Interface. Он отвечает за выполнение следующих задач:
 *          - Формирование информационных строк для категорий и пользовательских альбомов.
 *          - Удаление изображений и связанных данных.
 *          - Получение данных о новостях, языках и темах.
 *          - Генерация блоков вывода изображений для различных типов запросов.
...
class Work_CoreLogic implements Work_CoreLogic_Interface
{
     ...
     * @brief   Формирует информационную строку для категории или пользовательского альбома.
     *
     * @details Метод выполняет запросы к базе данных для получения информации о категории или пользовательском
     *          альбоме.
     *          Процесс включает следующие шаги:
     *          1. Получение данных о категории из таблицы `TBL_CATEGORY` или данных пользователя из таблицы
     *          `TBL_USERS`
     *             (если `$user_flag = 1`).
     ...
     *                          альбоме (`1`). По умолчанию: `0`. Допустимые значения: `0` или `1`.
     *
     * @return array Информационная строка для категории или пользовательского альбома:
     *               - 'name'           (string): Название категории или альбома.
     *               - 'description'    (string): Описание категории или альбома.
     ...
     * print_r($category_photo);
     * @endcode
     * @see     PhotoRigma::Classes::Work_CoreLogic::_create_photo_internal() Защищённый метод, реализующий основную
     *          логику.
     * @see     PhotoRigma::Classes::Work_CoreLogic::$config Свойство, содержащее конфигурацию приложения.
     * @see     PhotoRigma::Classes::Work_CoreLogic::$user Свойство, содержащее данные текущего пользователя.
     * @see     PhotoRigma::Classes::Work_CoreLogic::generate_photo_data() Приватный метод для формирования массива
     *          данных по умолчанию.
     * @see     PhotoRigma::Classes::Work::size_image() Метод, используемый для вычисления размеров изображения.
     * @see     PhotoRigma::Include::log_in_file() Функция для логирования ошибок.
     * @see     PhotoRigma::Classes::Work::clean_field() Метод для очистки данных.
     ...
     *          3. Проверяет существование файла изображения и его доступность.
     *          4. Вычисляет размеры изображения через метод `size_image()`.
     *          5. Возвращает массив данных для вывода изображения или вызывает `generate_photo_data()` в случае
     *          ошибки.
     *
     ...
     *      Пример сообщения:
     *          Неверное значение параметра $photo_id | Ожидалось положительное целое число
     * @throws RuntimeException|Exception Если возникает ошибка при выполнении запросов к базе данных или удалении
     *                                    файлов. Пример сообщения: Не удалось найти изображение | Переменная $photo_id
     *                                    = [значение]
     *
     * @warning Метод чувствителен к правам доступа при удалении файлов. Убедитесь, что скрипт имеет необходимые права
     *          на запись и чтение.
     ...
     * @see     PhotoRigma::Classes::Work_CoreLogic::_del_photo_internal() Защищённый метод, реализующий основную
     *          логику.
     * @see     PhotoRigma::Classes::Work_CoreLogic::$db Свойство, содержащее объект для работы с базой данных.
     * @see     PhotoRigma::Classes::Work_CoreLogic::$config Свойство, содержащее конфигурацию приложения.
     ...
     * @see     PhotoRigma::Classes::Database::aff_rows Свойство, содержащее количество затронутых строк после
     *          выполнения запроса.
     * @see     PhotoRigma::Include::log_in_file() Функция для логирования ошибок.
     *
     */
     ...
     *
     * @see     PhotoRigma::Classes::Work_CoreLogic::_get_languages_internal() Защищённый метод, реализующий основную
     *          логику.
     * @see     PhotoRigma::Classes::Work_CoreLogic::$config Свойство, содержащее конфигурацию приложения, включая путь
     *          к директории (`site_dir`).
     */
    public function get_languages(): array
    {
     ...
     *
     * @see     PhotoRigma::Classes::Work_CoreLogic::_get_themes_internal() Защищённый метод, реализующий основную
     *          логику.
     * @see     PhotoRigma::Include::log_in_file() Функция для логирования ошибок.
     */
    public function get_themes(): array
     ...
     * echo "Средняя оценка: {$averageRate}";
     * @endcode
     * @see     PhotoRigma::Classes::Work_CoreLogic::_process_rating_internal() Защищённый метод, реализующий основную
     *          логику.
     * @see     PhotoRigma::Classes::Work::process_rating() Фасад в родительском классе, через который вызывается этот
     *          метод.
     *
     */
    public function process_rating(string $table, int $photo_id, int $user_id, int $rate_value): float
