 ...
 *
 * @details   Этот интерфейс определяет контракт для классов, реализующих методы добавления, обновления и удаления
 *            пользователей и групп. Он должен обеспечивать:
...
interface User_Interface
{
    ...
    /**
     * @brief   Публичный метод для мягкого или окончательного удаления пользователя.
     *
     * @details Метод является частью контракта и определяет следующие обязательства:
    ...
    /**
     * @brief   Публичный метод для окончательного обезличивания и удаления пользователя.
     *
     * @details Метод является частью контракта интерфейса и определяет требования к реализации.
     ...
     *
     * @details Этот метод является частью контракта интерфейса и должен быть реализован в классе.
     *          Он предназначен для выполнения фонового процесса обезличивания пользователей,
     *          у которых истёк срок восстановления после мягкого удаления.
     *
     ...
     *
     * @details Метод является частью контракта и должен быть реализован в классе.
     *          Он обязан:
     ...
     * Пример использования:
     * @code
     * // Через POST-запрос из админки
     * $groupId = $user->add_new_group($_POST);
     * echo "Группа создана с ID: " . $groupId;
     * @endcode
     */
