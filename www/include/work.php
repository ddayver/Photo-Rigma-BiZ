...
class Work
{
    ...
    /**
     * @brief Получение данных о категории или пользовательском альбоме.
     *
     * @details Этот метод является делегатом (метод-редирект) для метода category() класса Work_CoreLogic.
     *          Метод выполняет запросы к базе данных для получения информации о категории или пользовательском альбоме,
     ...
     *      Метод для получения данных о категории.
     * @see PhotoRigma::Classes::Work_Helper::clean_field()
     *      Метод для очистки полей данных.
     *
     ...
     *
     * @details Этот метод является делегатом (метод-редирект) для метода create_photo() класса Work_Image.
     ...
     * @callgraph
     *
     * @see PhotoRigma::Classes::Work_Image::create_photo()
     *      Публичный метод-редирект для вызова этой логики.
     * @see PhotoRigma::Include::log_in_file()
     *      Функция для логирования ошибок.
     *
     ...
     * // Пример вызова метода через родительский класс Work
     * $work = new \PhotoRigma\Classes\Work();
     * $imageData = $work->create_photo('top', null);
     * print_r($imageData);
     * @endcode
     */
     ...
     *       Для инкапсуляции доступа к сессии рекомендуется заменить её на метод или свойство класса.
     *
