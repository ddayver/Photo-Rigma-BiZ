...
/**
 * @file        include/Work_Security_Interface.php
 ...
 *
 * @details     Этот файл содержит интерфейс `Work_Security_Interface`, который определяет методы для обеспечения
 *              безопасности приложения. Классы, реализующие этот интерфейс, предоставляют функционал для защиты от
 *              распространённых угроз, таких как SQL-инъекции, XSS-атаки, спам-боты и другие:
 ...
 *              - Проверка URL на наличие вредоносного кода.
 *              - Проверка входных данных из различных источников.
 *              - Проверка содержимого полей на соответствие регулярным выражениям или другим условиям.
 *              - Генерация математических CAPTCHA-вопросов для защиты от спам-ботов.
 *              - Фильтрация email-адресов для защиты от парсинга ботами.
 *
 * @note        Этот файл является частью системы PhotoRigma и играет ключевую роль в обеспечении безопасности
 *              приложения. Все методы данного интерфейса рекомендуется вызывать через класс `Work`.
 *
 ...
 * @brief     Интерфейс для работы с безопасностью.
 *
 * @details   Этот интерфейс определяет методы, которые должны быть реализованы классами, отвечающими за безопасность
 *            приложения. Все методы вызываются через класс `Work`. Интерфейс охватывает такие задачи, как:
 ...
 *            Методы интерфейса предназначены для обеспечения безопасности приложения и защиты от распространённых
 *            угроз, таких как SQL-инъекции, XSS-атаки, спам-боты и другие.
 *
...
interface Work_Security_Interface
{
     ...
     * @code
     * // Создание объекта
     * $securityHelper = new Work_Security();
     *
     * // Проверка безопасности URL
     * $is_safe = $securityHelper->url_check();
     * if ($is_safe) {
     *     echo "URL безопасен.";
     * } else {
     ...
     * $is_valid = $securityHelper->check_input('_POST', 'username', [
     *     'isset' => true,
     *     'empty' => true,
     * ]);
     * if ($is_valid) {
     ...
     * $regexp = "/^[a-z0-9]+$/i";
     * $is_valid = $securityHelper->check_field($field_value, $regexp, true);
     * if ($is_valid) {
     ...
     * $captcha = $securityHelper->gen_captcha();
     ...
     * $filtered_email = $securityHelper->filt_email($email);
