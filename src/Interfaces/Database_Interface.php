 ...
 *
 * Этот файл содержит определение интерфейса `Database_Interface`, который устанавливает контракт для классов,
 * предназначенных для работы с реляционными базами данных (MySQL/MariaDB, PostgreSQL, SQLite) через PDO. Интерфейс
 * гарантирует единообразие и предсказуемость поведения реализаций и включает методы для:
 * - Выполнения стандартных операций с данными (SELECT, JOIN, INSERT, UPDATE, DELETE, TRUNCATE) с поддержкой
 *   подготовленных выражений и **гибких опций запроса, включая сложные условия WHERE (OR, NOT)**.
 * - **Реализации полнотекстового поиска (Full-Text Search - FTS)**, специфичного для каждой поддерживаемой СУБД.
 ...
 *
 * @uses      \PhotoRigma\Interfaces\Cache_Handler_Interface Интерфейс обработчика кеша, используемый реализациями
 *                                                           Database_Interface.
 *
 ...
 * Интерфейс для работы с базами данных через PDO.
 *
 * Этот интерфейс определяет контракт для классов, предоставляющих функционал для работы с реляционными базами данных
 * через PDO, поддерживая следующие СУБД: MySQL (включая MariaDB), PostgreSQL и SQLite. Интерфейс гарантирует наличие
 * методов для выполнения основных операций с базой данных и предоставления служебной информации:
 * - Выполнение стандартных операций с данными (`SELECT`, `INSERT`, `UPDATE`, `DELETE`, `TRUNCATE`, `JOIN`) с
 *   поддержкой подготовленных выражений и гибких опций запроса. Ожидается, что методы, принимающие массив опций,
 *   будут поддерживать построение сложных условий `WHERE` с использованием операторов `OR` и `NOT`.
 * - Реализация **полнотекстового поиска (Full-Text Search - FTS)**. Интерфейс предусматривает наличие методов для
 *   выполнения FTS, которые должны быть реализованы с учетом специфики поддерживаемых СУБД и, как правило, включать
 *   механизмы fallback (например, на `LIKE`/`ILIKE`).
 ...
 * - Механизм временного изменения формата SQL для совместимости запросов между разными СУБД (например, возможность
 *   передать запрос в синтаксисе одной СУБД, который будет выполнен на другой).
 * - Интеграция с обработчиком кеша (`Cache_Handler_Interface`) для долгосрочного хранения оптимизационных данных
 *   класса-реализации.
 *
 * Все методы интерфейса должны использовать подготовленные выражения для обеспечения защиты от SQL-инъекций и
 * выбрасывать соответствующие исключения при возникновении ошибок.
 * Интерфейс может быть реализован различными классами, включая стандартный класс `Database` или пользовательские
 * реализации, следующие данному контракту.
 *
...
interface Database_Interface
{
     ...
     * Реализующие классы должны обеспечить корректную обработку всех входных параметров и возвращаемого значения
     * согласно описанию.
     *
    ...
    /**
     * Выполняет SQL-запрос с использованием JOIN для выборки данных.
     *
     * Этот метод является частью контракта интерфейса и должен быть реализован в классе.
     * Он определяет функциональность выполнения SQL-запроса с использованием JOIN, включая:
     * 1. Приём параметров для определения выбираемых полей, основной таблицы, массива JOIN-операций и дополнительных
     *    опций запроса (`where`, `group`, `order`, `limit`, `params`).
     ...
     *
     * Этот метод является частью контракта интерфейса и должен быть реализован в классе.
     * Он определяет функциональность полной очистки таблицы в базе данных с использованием операции TRUNCATE.
     *
     ...
     * 1. Приём массива данных для обновления, имени таблицы и опций запроса, включая обязательное условие `where`.
    ...
    /**
     * Извлекает одну строку результата.
     *
     * Этот метод является частью контракта интерфейса и должен быть реализован в классе.
     * Он определяет функциональность получения одной строки результата из последнего выполненного SQL-запроса
     * (обычно SELECT) в виде ассоциативного массива.
     * Реализация должна возвращать ассоциативный массив данных одной строки или `false`, если результатов больше нет.
     *
    ...
    /**
     * Извлекает все строки результата.
     *
     * Этот метод является частью контракта интерфейса и должен быть реализован в классе.
     * Он определяет функциональность получения всех строк результата из последнего выполненного SQL-запроса (обычно
     * SELECT) в виде массива ассоциативных массивов.
     ...
     *
     * Этот метод является частью контракта интерфейса и должен быть реализован в классе.
     * Он определяет функциональность получения количества строк, затронутых последним выполненным SQL-запросом на
     * модификацию данных (INSERT, UPDATE, DELETE, REPLACE).
    ...
    /**
     * Формирует SQL-выражение для форматирования даты.
     *
     * Этот метод является частью контракта интерфейса и должен быть реализован в классе.
     * Он определяет функциональность формирования специфичного для базы данных SQL-выражения для форматирования
     * значения из столбца с датой в заданный формат.
     *
     ...
     *
     * Этот метод является частью контракта интерфейса и должен быть реализован в классе.
     * Он определяет функциональность выполнения предоставленного коллбэка в контексте временно измененного формата
     * SQL, гарантируя восстановление исходного формата после завершения коллбэка, в том числе при возникновении
     * исключений.
     *
     ...
     * 1. Приём параметров для определения возвращаемых столбцов, столбцов для поиска, строки поиска, таблицы и
