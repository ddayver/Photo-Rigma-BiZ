 ...
 *
 * Этот файл содержит реализацию класса `Cache_Handler`, который предоставляет методы для работы с различными типами
 * кеширования:
 ...
 *
 * @uses      \PhotoRigma\Interfaces\Cache_Handler_Interface Интерфейс, который реализует класс.
 *
 ...
 * Класс для работы с системами кеширования.
 *
 * Этот класс предоставляет функционал для работы с различными типами кеширования:
 * - Файловое кеширование (file).
 * - Redis.
 * - Memcached.
 * Класс обеспечивает:
...
{
    /** @var \Redis|\Memcached|string|null Клиент для Redis/Memcached или путь к директории кеша */
    private Redis|Memcached|string|null $client;
     ...
     *                      - string `type`: Тип кеширования (`file`, `redis`, `memcached`). Если тип не указан,
     *                        используется значение по умолчанию: `file`. Если указан неизвестный тип, выбрасывается
     *                        исключение `InvalidArgumentException`.
     *                      - string `cache_dir` (опционально): Путь к директории кеша (для файлового кеширования).
     *                        Обязательный параметр для типа `file`. Если путь некорректен или недоступен, это
     *                        должно быть обработано в методе `_init_cache_file()`.
     *                      - string `host` (опционально): Хост для Redis/Memcached. Обязательный параметр для типов
     *                        `redis` и `memcached`.
     *                      - int `port` (опционально): Порт для Redis/Memcached. Если порт не указан, используются
     ...
     *
     * @uses \PhotoRigma\Classes\Cache_Handler::_init_cache_file() Метод для инициализации файлового кеширования.
     * @uses \PhotoRigma\Classes\Cache_Handler::_init_cache_redis() Метод для инициализации кеширования через Redis.
     * @uses \PhotoRigma\Classes\Cache_Handler::_init_cache_memcached() Метод для инициализации кеширования через
     *                                                                  Memcached.
     */
    ...
    /**
     * Инициализация файлового кеширования.
     *
     ...
     * 3. Проверяет безопасность пути (запрещает использование `../` для предотвращения выхода за пределы директории).
     ...
     *          Несоблюдение этих условий может привести к ошибкам инициализации.
     *
     * @uses \PhotoRigma\Classes\Cache_Handler::$client Свойство, содержащее путь к директории кеша после инициализации.
     */
     ...
     *
     * @param array $config Массив с настройками кеширования:
     *                      - string `host` (опционально): Хост Redis. Если не указан, используется значение по
     *                        умолчанию: `127.0.0.1`.
     *                      - int `port` (опционально): Порт Redis. Если не указан, используется значение по
     *                        умолчанию: `6379`.
     *
     * @throws RuntimeException Выбрасывается, если:
     ...
     *          Несоблюдение этих условий может привести к ошибкам инициализации.
     *
     * @uses \PhotoRigma\Classes\Cache_Handler::$client Свойство, содержащее экземпляр клиента Redis после
     *                                                  инициализации.
     */
     ...
     *
     * @param array $config Массив с настройками кеширования:
     *                      - string `host` (опционально): Хост Memcached. Если не указан, используется значение по
     *                        умолчанию: `127.0.0.1`.
     *                      - int `port` (опционально): Порт Memcached. Если не указан, используется значение по
     *                        умолчанию: `11211`.
     *
     * @throws RuntimeException Выбрасывается, если:
     ...
     *          Несоблюдение этих условий может привести к ошибкам инициализации.
     *
     * @uses \PhotoRigma\Classes\Cache_Handler::$client Свойство, содержащее экземпляр клиента Memcached после
     *                                                  инициализации.
     */
     ...
     *
     * Этот метод выполняет проверку актуальности данных в зависимости от типа кеширования:
     ...
     *
     * @uses \PhotoRigma\Classes\Cache_Handler::_is_valid_internal() Защищённый метод, который реализует логику
     *                                                               проверки актуальности данных в кеше.
     */
     ...
     * Метод является защищённым (protected) и предназначен для использования внутри класса или его наследников.
     * Вызывается из публичного метода `is_valid` как часть логики работы с кешем.
     *
     * @protected
     *
     * @param string     $key      Ключ кеша. Используется для проверки актуальности данных.
     ...
     *          Несоблюдение этих условий может привести к ошибкам проверки актуальности.
     *
     * @uses \PhotoRigma\Classes\Cache_Handler::_is_valid_file() Метод для проверки актуальности данных в файловом кеше.
     * @uses \PhotoRigma\Classes\Cache_Handler::_is_valid_storage() Метод для проверки актуальности данных в
    ...
    /**
     * Проверяет актуальность данных в файловом кеше.
     *
     * Этот метод выполняет следующие действия:
     ...
     * 4. Извлекает значения переменных `$checksum` и данные из содержимого файла с помощью регулярных выражений.
    ...
    /**
     * Проверяет актуальность данных в хранилище (Redis/Memcached).
     *
     * Этот метод выполняет следующие действия:
     * 1. Проверяет, что клиент хранилища инициализирован и является экземпляром класса `Redis` или `Memcached`.
     *    - Если клиент не инициализирован или имеет неправильный тип, выбрасывается исключение `RuntimeException`.
     * 2. Получает сохранённую контрольную сумму из хранилища по ключу `$key . '_checksum'`.
     *    - Если контрольная сумма отсутствует или не совпадает с переданной (`$checksum`), возвращается `false`.
     ...
     *
     * Этот метод выполняет запись данных в зависимости от типа кеширования:
     * - Если тип кеширования — `file`, используется файловый кеш.
     * - Если тип кеширования — `redis` или `memcached`, используется хранилище (Redis/Memcached).
     * - Если тип кеширования не поддерживается, возвращается `false`.
     * Метод является публичным и предназначен для использования вне класса.
     ...
     *
     * @uses \PhotoRigma\Classes\Cache_Handler::_update_cache_internal() Защищённый метод, который реализует логику
     *                                                                   записи данных в кеш.
     */
     ...
     * 3. Если тип кеширования не поддерживается, возвращается `false`.
     * Метод является защищённым (protected) и предназначен для использования внутри класса или его наследников.
     * Вызывается из публичного метода `update_cache` как часть логики работы с кешем.
     *
     * @protected
     *
     * @param string     $key      Ключ кеша. Используется для записи данных.
     ...
     *          Несоблюдение этих условий может привести к ошибкам записи данных.
     *
     * @uses \PhotoRigma\Classes\Cache_Handler::_update_cache_file() Метод для записи данных в файловый кеш.
     * @uses \PhotoRigma\Classes\Cache_Handler::_update_cache_storage() Метод для записи данных в хранилище
     *                                                                  (Redis/Memcached).
     */
    ...
    /**
     * Записывает данные в файловый кеш.
     *
     * Этот метод выполняет следующие действия:
     * 1. Формирует путь к файлу кеша на основе ключа `$key`.
     *    - Путь формируется как: `$this->client . $key . '.php'`.
    ...
    /**
     * Записывает данные в хранилище (Redis/Memcached).
     *
     * Этот метод выполняет следующие действия:
