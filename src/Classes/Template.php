...
/**
 * Класс для работы с HTML-шаблонами, включая рендеринг и подстановку данных.
 *
 * Этот файл содержит реализацию класса `Template`, который предоставляет методы для работы с HTML-шаблонами:
 ...
 *            - Поддержка читаемых URL через параметр `$mod_rewrite`.
 *
 * @uses      \PhotoRigma\Classes\Template_Interface Интерфейс для работы с шаблонами.
 *
 ...
 *
 * Этот класс реализует интерфейс `Template_Interface` и предоставляет функционал для работы с HTML-шаблонами.
 ...
 * Все ошибки, возникающие при работе с шаблонами, обрабатываются через исключения.
 *
 * @property-write string $ins_header    Данные, вставляемые в заголовок.
 * @property-write string $template_file Файл шаблона.
 * @property-read  string $content       Содержимое для вывода.
 *
 * @uses \PhotoRigma\Interfaces\Template_Interface Интерфейс, который реализует класс.
 */
class Template implements Template_Interface
{
    /** @var string Данные, вставляемые в заголовок */
    private string $ins_header = '';
    /** @var string Содержимое для вывода */
    private string $content = '';
    /** @var string Файл шаблона */
    private string $template_file = 'main.html';
    /** @var array Блок массивов объектов для обработки */
    private array $block_object = [];
    /** @var array Блок строковых данных для замены */
    private array $block_string = [];
    /** @var array Блок условий для обработки */
    private array $block_if = [];
    /** @var array Блок массивов выбора блока для обработки */
    private array $block_case = [];
    /** @var string Ссылка на корень темы */
    private string $themes_url;
    /** @var array Массив с путями к папкам шаблонов */
    private array $template_dirs;
    /** @var string Тема пользователя */
    private string $theme;
    /** @var \PhotoRigma\Interfaces\Work_Interface|null Свойство для объекта класса Work_Interface */
    private ?Work_Interface $work = null;

    /**
     * Конструктор класса, инициализирующий параметры темы оформления.
     *
     * Этот метод:
     * - Проверяет корректность имени темы ($theme)
     * - Инициализирует пути к темам ($template_dirs)
     * - Формирует URL темы ($themes_url)
     * - Проверяет наличие файла шаблона по умолчанию через _find_template_file()
     * Если проверка не пройдена — выбрасывается InvalidArgumentException
     *
     * @param array  $template_dirs Массив с путями к папкам шаблонов.
     *                              Должен содержать полные пути к директориям тем.
     * @param string $theme         Имя темы оформления.
     *                              Допустимые символы: латинские буквы, цифры, дефисы и подчеркивания.
     *                              Пример: 'default'
     *                              Ограничение: не должно быть пустым и должно соответствовать регулярному выражению
     *                              `/^[a-zA-Z0-9_-]+$/`.
     *
     * @return void
     *
     * @note    Для работы требуется, чтобы все указанные пути в $template_dirs существовали и были доступны для чтения
     *          После инициализации URL темы формируется как SITE_URL . 'themes/' . $theme . '/'
     *
     * @warning Не передавайте пустое значение $theme — это вызовет исключение
     *          Убедитесь, что пути в $template_dirs корректны — иначе шаблон не будет найден
     *          Не используйте русские или специальные символы в имени темы
     *
     * @uses \PhotoRigma\Classes\Template::_find_template_file() Для проверки наличия шаблона
     */
    ...
    /**
     * Проверяет, является ли файл шаблона корректным и доступным для чтения.
     *
     * Метод:
     * - Перебирает пути из $template_dirs
     * - Строит полный путь к файлу шаблона на основе имени темы
     * - Ищет первый доступный для чтения файл
     * - Возвращает его путь или выбрасывает исключение
     *
     * @internal
     *
     * @param string $template_file Имя файла шаблона (без пути)
     *                              Пример: 'main.html'
     *
     * @return string Полный путь к найденному файлу шаблона
     *
     * @throws RuntimeException Если ни один файл не найден или нечитаем
     *
     * @note   Метод использует свойства $template_dirs и $theme для построения путей
     * @note   Поддерживает поиск в нескольких директориях — в порядке указания в $template_dirs
     *
     * @warning Убедитесь, что хотя бы одна директория из $template_dirs содержит корректный шаблон
     *          с заданным именем и правами на чтение
     *
     * @uses \PhotoRigma\Classes\Template::$template_dirs Свойство, хранящее пути к темам
     * @uses \PhotoRigma\Classes\Template::$theme Свойство, хранящее имя текущей темы
     */
    ...
    /**
     * Магический метод для получения значения свойства `$content`.
     *
     * Этот метод вызывается автоматически при попытке получить значение недоступного свойства. Доступ разрешён
     * только к свойству `$content`. Если запрашивается другое свойство, выбрасывается исключение
     ...
     *
     * @uses \PhotoRigma\Classes\Template::$content Свойство, содержащее содержимое для вывода.
     */
    ...
    /**
     * Устанавливает значение приватных свойств `$template_file` и `$ins_header`.
     *
     * Этот метод позволяет изменить значения приватных свойств `$template_file` и `$ins_header`.
     * Алгоритм работы:
     ...
     *
     * @uses \PhotoRigma\Classes\Template::$template_file Свойство, содержащее имя файла шаблона.
     * @uses \PhotoRigma\Classes\Template::$ins_header Свойство, вставляемое в заголовок.
     * @uses \PhotoRigma\Classes\Template::_find_template_file() Метод, проверяющий правильность переданного имени
     *                                                           файла шаблона.
     */
    ...
    /**
     * Установка объекта, реализующего интерфейс Work_Interface, через сеттер.
     *
     * Этот метод позволяет установить объект, реализующий интерфейс `Work_Interface`. Присваивает объект свойству
     ...
     *
     * @uses \PhotoRigma\Classes\Work_Interface Интерфейс, которому должен соответствовать объект.
     */
    ...
    /**
     * Добавляет массив условий для вывода фрагментов шаблона через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _add_if_array_internal(). Он добавляет массив
     * данных об условиях вывода фрагментов шаблона, с возможностью рекурсивного размещения. Метод проверяет
     * корректность входных данных и использует метод `add_if()` для добавления каждой пары ключ-значение.
     ...
     *
     * @uses \PhotoRigma\Classes\Template::_add_if_array_internal() Защищённый метод, реализующий основную логику
     *                                                              добавления условий вывода фрагментов шаблона.
     */
    ...
    /**
     * Метод добавляет массив данных об условиях вывода фрагментов шаблона, с возможностью рекурсивного размещения.
     *
     ...
     * 3. Для каждого элемента массива проверяет корректность ключа (только латинские буквы, цифры и подчеркивания) и
     *    значения (булево значение).
     ...
     * логика вызывается через публичный метод-редирект `add_if_array()`.
     *
     * @protected
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Template::_add_if_internal() Метод, используемый для добавления отдельных условий.
     */
    ...
    /**
     * Добавляет условие для вывода фрагментов шаблона через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _add_if_internal(). Он добавляет данные об
     * условиях вывода фрагментов шаблона, с возможностью рекурсивного размещения. Метод проверяет корректность
     * входных данных и добавляет условие в массив `block_if` или объекты в зависимости от пути. Предназначен для
     * прямого использования извне.
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Template::_add_if_internal() Защищённый метод, реализующий основную логику
     *                                                        добавления условий вывода фрагментов шаблона.
     */
    ...
    /**
     * Метод добавляет данные об условиях вывода фрагментов шаблона, с возможностью рекурсивного размещения.
     *
     * Этот метод выполняет следующие шаги:
     * 1. Проверяет корректность имени условия (только латинские буквы, цифры и подчеркивания). Если имя некорректно,
     *    выбрасывается исключение.
     * 2. Проверяет формат пути (`$path_array`), если он указан. Путь должен быть строкой или `false`.
     * 3. Если путь не указан, добавляет условие в массив `block_if`. Если путь указан, рекурсивно добавляет условие
     *    в объекты с использованием метода `_test_is_object()`. Этот метод является защищенным и предназначен для
     ...
     *          Некорректные данные могут привести к выбросу исключения.
     *
     * @uses \PhotoRigma\Classes\Template::$block_if Свойство, содержащее массив условий.
     * @uses \PhotoRigma\Classes\Template::$block_object Свойство, содержащее объекты для рекурсивного размещения
     *                                                   условий.
     * @uses \PhotoRigma\Classes\Template::_test_is_object() Метод, используемый для разбора пути.
     */
    ...
    /**
     * Метод проверяет путь, разбирает его на составные части и создает новый объект-массив при необходимости.
     *
     * Этот метод выполняет следующие шаги:
     ...
     *
     * @uses \PhotoRigma\Classes\Template Класс, используемый для создания новых объектов.
     * @uses \PhotoRigma\Classes\Template::$block_object Свойство, содержащее массив объектов.
     */
    ...
    /**
     * Добавляет массив данных о выборе блока для вывода фрагментов шаблона через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _add_case_array_internal(). Он добавляет массив
     * данных о выборе блока для вывода фрагментов шаблона, с возможностью рекурсивного размещения. Метод проверяет
     * корректность входных данных и использует метод `add_case()` для добавления каждой пары ключ-значение.
     * Предназначен для прямого использования извне.
     *
     ...
     *          Некорректные данные могут привести к выбросу исключения.
     *
     * @uses \PhotoRigma\Classes\Template::_add_case_array_internal() Защищённый метод, реализующий основную логику
     *                                                                добавления данных о выборе блока.
     */
    ...
    /**
     * Метод добавляет массив данных о выборе блока для вывода фрагментов шаблона, с возможностью рекурсивного
     * размещения.
     *
     * Этот метод выполняет следующие шаги:
     * 1. Проверяет, что массив данных не пуст. Если массив пуст, выбрасывается исключение.
     * 2. Проверяет формат пути (`$path_array`), если он указан. Путь должен быть строкой или `false`.
     * 3. Для каждого элемента массива проверяет корректность ключа и значения (только латинские буквы, цифры и
     *    подчеркивания). Если ключ или значение некорректны, выбрасывается исключение.
     * 4. Добавляет каждую пару ключ-значение в шаблон с помощью метода `add_case()`.
     * Этот метод является защищенным и предназначен для использования внутри класса или его наследников. Основная
     * логика вызывается через публичный метод-редирект `add_case_array()`.
     *
     * @protected
     *
     ...
     *          Некорректные данные могут привести к выбросу исключения.
     *
     * @uses \PhotoRigma\Classes\Template::_add_case_internal() Метод, используемый для добавления отдельных условий
     *                                                          выбора блока.
     */
    ...
    /**
     * Добавляет данные о выборе блока для вывода фрагментов шаблона через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _add_case_internal(). Он добавляет данные о
     * выборе блока для вывода фрагментов шаблона, с возможностью рекурсивного размещения. Метод проверяет
     * корректность входных данных и добавляет условие в массив `block_case` или объекты в зависимости от пути.
     * Предназначен для прямого использования извне.
     *
     * @param string       $name       Название условия:
     *                                 - Должно содержать только латинские буквы, цифры и подчеркивания.
     ...
     *          Некорректные данные могут привести к выбросу исключения.
     *
     * @uses \PhotoRigma\Classes\Template::_add_case_internal() Защищённый метод, реализующий основную логику
     *                                                          добавления данных о выборе блока.
     */
    ...
    /**
     * Метод добавляет данные о выборе блока для вывода фрагментов шаблона, с возможностью рекурсивного размещения.
     *
     * Этот метод выполняет следующие шаги:
     * 1. Проверяет корректность имени условия (только латинские буквы, цифры и подчеркивания). Если имя некорректно,
     *    выбрасывается исключение.
     * 2. Проверяет корректность значения условия (только латинские буквы, цифры и подчеркивания). Если значение
     ...
     *    условие в объекты с использованием метода `_test_is_object()`.
     * Этот метод является защищенным и предназначен для использования внутри класса или его наследников. Основная
     * логика вызывается через публичный метод-редирект `add_case()`.
     *
     * @protected
     *
     * @param string       $name       Название условия. Должно содержать только латинские буквы, цифры и
     ...
     *
     * @uses \PhotoRigma\Classes\Template::$block_case Свойство, содержащее массив данных о выборе блока.
     * @uses \PhotoRigma\Classes\Template::$block_object Свойство, содержащее объекты для рекурсивного размещения
     *                                                   условий.
     * @uses \PhotoRigma\Classes\Template::_test_is_object() Метод, используемый для разбора пути.
     */
    ...
    /**
     * Формирует заголовок HTML-страницы с меню, метаданными и блоками контента через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _page_header_internal(). Он формирует заголовок
     * HTML-страницы, включая меню, метаданные и блоки контента, на основе переданных параметров. Метод проверяет
     * корректность входных данных, генерирует данные для меню и фотографий, и добавляет контент в начало страницы.
     * Предназначен для прямого использования извне.
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Template::_page_header_internal() Защищённый метод, реализующий основную логику
     *                                                             формирования заголовка HTML-страницы.
     */
    ...
    /**
     * Метод формирует заголовок HTML-страницы с меню, метаданными и блоками контента.
     *
     * Этот метод выполняет следующие шаги:
     ...
     * 3. Создает экземпляр шаблона заголовка (`header.html`) и заполняет его строковыми данными, условиями и выбором
     *    блоков.
     ...
     * Этот метод является защищенным и предназначен для использования внутри класса или его наследников. Основная
     * логика вызывается через публичный метод-редирект `page_header()`.
     *
     * @protected
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Template::$content Свойство, содержащее текущее содержимое страницы.
     * @uses \PhotoRigma\Classes\Template::add_if() Метод, используемый для добавления условий вывода фрагментов
     *                                              шаблона.
     * @uses \PhotoRigma\Classes\Template::add_case() Метод, используемый для выбора блоков для вывода фрагментов
     *                                                шаблона.
     * @uses \PhotoRigma\Classes\Template::add_string_array() Метод, используемый для передачи массива строк в шаблон
     * @uses \PhotoRigma\Classes\Template::add_string() Метод, используемый для передачи строк в шаблон
     * @uses \PhotoRigma\Classes\Template::create_template() Метод, используемый для создания контента из шаблона.
     * @uses \PhotoRigma\Classes\Work::create_menu() Метод, используемый для генерации данных меню.
     * @uses \PhotoRigma\Classes\Work::create_photo() Метод, используемый для генерации данных фотографий.
     * @uses \PhotoRigma\Classes\Work::clean_field() Функция, используемая для очистки данных.
     */
    ...
    /**
     * Добавляет массив строковых данных для замены в шаблоне через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _add_string_array_internal(). Он добавляет
     * массив строковых данных для замены в шаблоне, с возможностью рекурсивного размещения. Метод проверяет
     * корректность входных данных и использует метод `add_string()` для добавления каждой пары ключ-значение.
     * Предназначен для прямого использования извне.
     *
     ...
     *          Некорректные данные могут привести к выбросу исключения.
     *
     * @uses \PhotoRigma\Classes\Template::_add_string_array_internal() Защищённый метод, реализующий основную логику
     *                                                                  добавления строковых данных для замены в
     *                                                                  шаблоне.
     */
    ...
    /**
     * Метод добавляет массив строковых данных для замены в шаблоне, с возможностью рекурсивного размещения.
     *
     * Этот метод выполняет следующие шаги:
     * 1. Проверяет, что массив данных не пуст. Если массив пуст, выбрасывается исключение.
     * 2. Проверяет формат пути (`$path_array`), если он указан. Путь должен быть строкой или `false`.
     * 3. Для каждого элемента массива проверяет корректность ключа (только латинские буквы, цифры и подчеркивания) и
     *    значения (строка). Если ключ или значение некорректны, выбрасывается исключение.
     * 4. Добавляет каждую пару ключ-значение в шаблон с помощью метода `add_string()`.
     * Этот метод является защищенным и предназначен для использования внутри класса или его наследников. Основная
     * логика вызывается через публичный метод-редирект `add_string_array()`.
     *
     * @protected
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Template::_add_string_internal() Метод, используемый для добавления отдельных
    ...
    /**
     * Добавляет строковую переменную для замены в шаблоне через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _add_string_internal(). Он добавляет строковую
     * переменную для замены в шаблоне, с возможностью рекурсивного размещения. Метод проверяет корректность входных
     * данных и добавляет переменную в массив `block_string` или объекты в зависимости от пути. Предназначен для
     * прямого использования извне.
     *
     ...
     *          Некорректные данные могут привести к выбросу исключения.
     *
     * @uses \PhotoRigma\Classes\Template::_add_string_internal() Защищённый метод, реализующий основную логику
     *                                                            добавления строковых переменных для замены в шаблоне.
     */
    ...
    /**
     * Метод добавляет строковую переменную для замены в шаблоне, с возможностью рекурсивного размещения.
     *
     * Этот метод выполняет следующие шаги:
     ...
     * 3. Если указан путь (`$path_array`), проверяет его формат (строка или `false`) и разбирает с помощью метода
     *    `_test_is_object()`. Если результат разбора некорректен, выбрасывается исключение.
     * 4. Если путь не указан, добавляет переменную в массив `block_string`. Если путь указан, рекурсивно добавляет
     *    переменную в объекты.
     * Этот метод является защищенным и предназначен для использования внутри класса или его наследников. Основная
     * логика вызывается через публичный метод-редирект `add_string()`.
     *
     * @protected
     *
     ...
     *          символы. Некорректные данные могут привести к выбросу исключения.
     *
     * @uses \PhotoRigma\Classes\Template::$block_string Свойство, содержащее массив строковых переменных.
     * @uses \PhotoRigma\Classes\Template::$block_object Свойство, содержащее объекты для рекурсивного размещения
     *                                                   переменных.
     * @uses \PhotoRigma\Classes\Template::_test_is_object() Метод, используемый для разбора пути.
     */
    ...
    /**
     * Создает и обрабатывает шаблон через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _create_template_internal(). Он выполняет чтение
     * файла шаблона, парсинг его содержимого и замену плейсхолдеров `{SITE_URL}` и `{THEME_URL}` на реальные
     * значения. Предназначен для прямого использования извне.
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Template::_create_template_internal() Защищённый метод, реализующий основную логику
     *                                                                 создания и обработки шаблона.
     */
    ...
    /**
     * Метод создает и обрабатывает шаблон, включая чтение файла, парсинг и замену плейсхолдеров.
     *
     * Этот метод выполняет следующие шаги:
     ...
     * Этот метод является защищенным и предназначен для использования внутри класса или его наследников. Основная
     * логика вызывается через публичный метод-редирект `create_template()`.
     *
     * @protected
     *
     * @throws RuntimeException Выбрасывается, если:
     ...
     *          что файл существует, доступен для чтения и содержит допустимые данные.
     *
     * @uses \PhotoRigma\Classes\Template::_pars_template() Метод, используемый для парсинга шаблона.
     * @uses \PhotoRigma\Classes\Work::clean_field() Внешняя функция, используемая для очистки данных.
     * @uses \PhotoRigma\Classes\Template::$template_file Свойство, содержащее путь к файлу шаблона.
     * @uses \PhotoRigma\Classes\Template::$content Свойство, содержащее содержимое шаблона после обработки.
     */
    ...
    /**
     * Метод обрабатывает шаблон для подготовки его к выводу: рекурсивно обрабатывает объектные блоки, условия
     ...
     * 2. Рекурсивно обрабатывает объектные блоки (`$this->block_object`) в цикле с помощью метода `_template_object()`.
     ...
     *
     * @uses \PhotoRigma\Classes\Template::$block_if Свойство, содержащее массив условий.
     * @uses \PhotoRigma\Classes\Template::$block_case Свойство, содержащее массив выбора блоков.
     * @uses \PhotoRigma\Classes\Template::$block_string Свойство, содержащее массив строковых переменных.
     * @uses \PhotoRigma\Classes\Template::$block_object Свойство, содержащее массив объектных блоков.
     * @uses \PhotoRigma\Classes\Template::_template_object() Метод, используемый для обработки объектных блоков.
     * @uses \PhotoRigma\Classes\Template::_template_if() Метод, используемый для обработки условий.
     * @uses \PhotoRigma\Classes\Template::_template_case() Метод, используемый для обработки выбора блоков.
     * @uses \PhotoRigma\Classes\Template::_url_mod_rewrite() Метод, используемый для модификации URL.
     */
    ...
    /**
     * Метод выполняет рекурсивную обработку блока массивов-объектов, подготавливая их содержимое для вывода между
     ...
     * 3. Использует регулярное выражение с флагом `/s` (single-line mode) для поиска всех блоков, заключенных между
     *    этими тегами.
     * 4. Для каждого элемента массива `$index` проверяет, что он является объектом и имеет метод `_pars_template()`.
     ...
     *
     * @uses \PhotoRigma\Classes\Template::$content Свойство, содержащее содержимое шаблона.
     */
    ...
    /**
     * Метод обрабатывает блоки условий вывода фрагментов шаблона, выбирая содержимое до `_ELSE` или после него в
     ...
     *    - Содержимое должно быть непустой строкой.
     *    - Если содержимое некорректно, выбрасывается исключение.
     * 2. Формирует ключи для поиска блоков в шаблоне: `<!-- IF_НАЗВАНИЕ_BEGIN -->`, `<!-- IF_НАЗВАНИЕ_ELSE -->` и
     *    `<!-- IF_НАЗВАНИЕ_END -->`.
     * 3. Использует регулярное выражение с флагом `/s` (single-line mode) для поиска всех блоков, заключенных между
     *    этими тегами.
     ...
     * 6. Заменяет найденные блоки в шаблоне на выбранное содержимое, удаляя лишние пробелы с помощью `trim()`.
    ...
    /**
     * Метод обрабатывает блок выбора фрагмента шаблона, выбирая один из блоков `CASE_ЗНАЧЕНИЕ` или `CASE_DEFAULT` в
     * зависимости от значения условия.
     *
     * Этот метод выполняет следующие шаги:
     * 1. Проверяет корректность содержимого шаблона (`$this->content`):
     ...
     *    Для его поиска используется `preg_match`.
    ...
    /**
     * Метод преобразует URL в "красивый" вид (например, `?action=profile&id=123` → `profile/id_123.html`) с
     * использованием правил `mod_rewrite`, если они включены.
     *
     * Этот метод выполняет следующие шаги:
     * 1. Проверяет, включен ли режим `mod_rewrite` (константа `URL_REWRITE`).
     *    - Если режим выключен, содержимое возвращается без изменений.
     * 2. Определяет шаблоны для замены URL на основе параметров запроса:
     *    - Шаблоны (`$patterns`) и замены (`$replacements`) работают в паре.
     *    - Например, `?action=profile&id=123` заменяется на `profile/id_123.html`.
     * 3. Выполняет замену URL в переданном содержимом (`$content`) с использованием регулярных выражений.
     * 4. Возвращает обработанное содержимое.
     * Метод является приватным и предназначен только для использования внутри класса.
     *
     * @internal
     *
    ...
    /**
     * Формирует подвал HTML-страницы через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _page_footer_internal(). Он формирует подвал
     * HTML-страницы, включая копирайт, статистику, информацию о пользователе и случайную фотографию. Метод проверяет
     * корректность входных данных, генерирует необходимые данные и добавляет контент в конец страницы. Предназначен
     * для прямого использования извне.
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Template::_page_footer_internal() Защищённый метод, реализующий основную логику
     *                                                             формирования подвала HTML-страницы.
     */
    ...
    /**
     * Метод формирует подвал HTML-страницы с выводом копирайта, статистики, информации о пользователе и случайной
     * фотографии.
     *
     * Этот метод выполняет следующие шаги:
     * 1. Генерирует данные для подвала, включая информацию о пользователе, статистику, лучших пользователей и
     *    случайную фотографию с помощью методов `template_user()`, `template_stat()`, `template_best_user()` и
     *    `create_photo()`.
     * 2. Создает экземпляр шаблона подвала (`footer.html`) и заполняет его строковыми данными, условиями и выбором
     *    блоков.
     ...
     * Этот метод является защищенным и предназначен для использования внутри класса или его наследников. Основная
     * логика вызывается через публичный метод-редирект `page_footer()`.
     *
     * @protected
     *
     * @param int    $login_id   Идентификатор пользователя, авторизованного на сайте.
     ...
     *                                                шаблона.
     * @uses \PhotoRigma\Classes\Template::create_template() Метод, используемый для создания контента из шаблона.
     * @uses \PhotoRigma\Classes\Work::template_user() Метод, используемый для генерации данных о пользователе.
     * @uses \PhotoRigma\Classes\Work::template_stat() Метод, используемый для генерации статистики.
     * @uses \PhotoRigma\Classes\Work::template_best_user() Метод, используемый для генерации данных о лучших
     *                                                      пользователях.
     * @uses \PhotoRigma\Classes\Work::create_photo() Метод, используемый для генерации данных случайной фотографии.
     * @uses \PhotoRigma\Classes\Work::clean_field() Функция, используемая для очистки данных.
     */
