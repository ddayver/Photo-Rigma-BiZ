...
/**
 * Файл содержит класс Work_Helper, который предоставляет вспомогательные методы для работы с данными.
 *
 * Этот файл содержит класс `Work_Helper`, который реализует вспомогательные методы для обработки данных. Класс
 * предоставляет методы для очистки строк от HTML-тегов, преобразования размеров в байты, транслитерации строк,
 * преобразования BBCode в HTML, разбиения строк на ограниченные по длине части и проверки MIME-типов. Все методы
 * класса являются статическими.
 *
 * @author    Dark Dayver
 * @version   0.5.0
 * @since     2025-05-29
 ...
 *
 * @uses      \PhotoRigma\Classes\Work Класс, через который вызываются методы для работы с данными.
 ...
 *
 * Этот класс реализует интерфейс `Work_Helper_Interface` и предоставляет статические методы для выполнения различных
 * вспомогательных задач. Он является дочерним классом для `\PhotoRigma\Classes\Work` и наследует его методы. Все
 * методы данного класса рекомендуется вызывать через родительский класс `Work`, так как их поведение может быть
 * непредсказуемым при прямом вызове. Основные возможности:
 ...
 * Методы класса предназначены для использования в различных частях приложения, таких как обработка пользовательского
 * ввода, работа с файлами, генерация безопасного вывода и т.д.
 *
 * @uses \PhotoRigma\Interfaces\Work_Helper_Interface Интерфейс, который реализует данный класс.
 */
class Work_Helper implements Work_Helper_Interface
{
    /**
     * Очистка строки от HTML-тегов и специальных символов через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _clean_field_internal().
     ...
     *
     * @uses \PhotoRigma\Classes\Work_Helper::_clean_field_internal() Защищённый метод, выполняющий очистку.
     */
    ...
    /**
     * Очистка строки от HTML-тегов и специальных символов (защищённый метод).
     *
     ...
     * 4. Экранирует специальные символы (например, `<`, `>`, `&`, `"`, `'`) с использованием `htmlspecialchars` и
     *    флагов `ENT_QUOTES | ENT_HTML5`.
     * Этот метод используется для защиты от XSS-атак и других проблем, связанных с некорректными данными. Он
     * является защищённым и предназначен для использования внутри класса или его наследников. Основная логика метода
    ...
    /**
     * Транслитерация строки и замена знаков пунктуации на "_" через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _encodename_internal().
     * Он выполняет транслитерацию не латинских символов в латиницу и заменяет знаки пунктуации на "_". Используется
     * для создания "безопасных" имён файлов или URL. Метод также доступен через метод-фасад `encodename()` в
     * родительском классе.
     *
     * Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется. В противном
     * случае поведение метода может быть непредсказуемым.
     ...
     *
     * @uses \PhotoRigma\Classes\Work_Helper::_encodename_internal() Защищённый метод, выполняющий транслитерацию.
     */
    ...
    /**
     * Транслитерация строки и замена знаков пунктуации на "_" (защищённый метод).
     *
     * Этот защищённый метод выполняет следующие действия:
     ...
     *    - Если расширение `intl` доступно, используется метод `Transliterator::create('Any-Latin; Latin-ASCII')`.
     ...
     * Этот метод используется для создания "безопасных" имен файлов или URL. Он является защищённым и предназначен
     * для использования внутри класса или его наследников. Основная логика метода вызывается через публичный метод
     * `encodename()`.
     *
     * @protected
     *
    ...
    /**
     * Преобразует размер в байты через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _return_bytes_internal().
     * Он преобразует размер, заданный в формате "число[K|M|G]", в количество байт. Метод также доступен через
     * метод-фасад `return_bytes()` в родительском классе.
     *
     * Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется. В противном
     * случае поведение метода может быть непредсказуемым.
     *
     ...
     *          Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Helper::_return_bytes_internal() Защищённый метод, выполняющий преобразование.
     */
    ...
    /**
     * Преобразование размера в байты (защищённый метод).
     *
     * Этот защищённый метод преобразует размер, заданный в формате "число[K|M|G]", в количество байт.
     ...
     * логика метода вызывается через публичный метод `return_bytes()`.
     *
     * @protected
     *
     * @param string|int $val Размер в формате "число[K|M|G]" или число.
    ...
    /**
     * Преобразует BBCode в HTML через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _ubb_internal().
     * Он преобразует BBCode-теги в HTML-теги. Метод также доступен через метод-фасад `ubb()` в родительском классе.
     *
     * Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется. В противном
     * случае поведение метода может быть непредсказуемым.
     *
     ...
     *          Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Helper::_ubb_internal() Защищённый метод, выполняющий преобразование BBCode.
     */
    ...
    /**
     * Преобразование BBCode в HTML (защищённый метод).
     *
     * Этот защищённый метод преобразует BBCode-теги в соответствующие HTML-теги с учетом рекурсивной
     ...
     * Этот метод является защищённым и предназначен для использования внутри класса или его наследников. Основная
     * логика метода вызывается через публичный метод `ubb()`.
     *
     * @protected
     *
     * @param string $text Текст с BBCode.
     ...
     *
     * @uses \PhotoRigma\Include\log_in_file() Записывает сообщение об ошибке в лог-файл.
     * @uses \PhotoRigma\Classes\Work_Helper::_process_bbcode_recursively() Приватный метод для рекурсивной обработки
     *                                                                      BBCode.
     */
    ...
    /**
     * Рекурсивно преобразует BBCode в HTML, проверяя глубину рекурсии и корректность тегов, а также защищая от
     * XSS-атак.
     *
     ...
     *    - Простые теги (например, `[b]`, `[i]`, `[u]`) преобразуются в HTML-теги (`<strong>`, `<em>`, `<u>`).
     *    - Сложные теги (например, `[url]`, `[img]`) включают дополнительные проверки (например, валидация URL).
     ...
     * 6. Логирует ошибки при обнаружении некорректных данных (например, невалидный URL) с помощью `log_in_file`.
     ...
     *
     * @uses \PhotoRigma\Include\log_in_file() Записывает сообщение об ошибке в лог-файл.
     * @uses \PhotoRigma\Classes\Work_Helper::_clean_field_internal() Метод для очистки данных (защита от XSS).
    ...
    /**
     * Разбивает строку на несколько строк ограниченной длины через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _utf8_wordwrap_internal().
     ...
     *          Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Helper::_utf8_wordwrap_internal() Защищённый метод, выполняющий разбиение строки.
     */
    ...
    /**
     * Разбивка строки на несколько строк ограниченной длины (защищённый метод).
     *
     * Этот защищённый метод разбивает строку на несколько строк, каждая из которых имеет длину не более указанной.
     * Разрыв строки выполняется только по пробелам, чтобы сохранить читаемость текста. Поддерживается работа с UTF-8
     * символами. Если параметры некорректны (например, $width <= 0 или $break пустой), возвращается исходная строка.
     * Этот метод является защищённым и предназначен для использования внутри класса или его наследников. Основная
     * логика метода вызывается через публичный метод `utf8_wordwrap()`.
     *
     * @protected
     *
     * @param string $str   Исходная строка.
    ...
    /**
     * Проверяет MIME-тип файла через доступные библиотеки через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _validate_mime_type_internal().
     ...
     *
     * @uses \PhotoRigma\Classes\Work_Helper::_validate_mime_type_internal() Защищённый метод, выполняющий проверку
     *                                                                       MIME-типа.
     */
    ...
    /**
     * Проверка MIME-типа файла через доступные библиотеки (защищённый метод).
     *
     * Этот защищённый метод проверяет, поддерживается ли указанный MIME-тип хотя бы одной из доступных библиотек:
     ...
     * Проверка выполняется последовательно: сначала через Imagick, затем через Gmagick, и в конце через GD. Если
     * MIME-тип не поддерживается ни одной библиотекой, возвращается `false`. Логируются случаи, когда MIME-тип не
     * поддерживается конкретной библиотекой.
     * Этот метод является защищённым и предназначен для использования внутри класса или его наследников. Основная
     * логика метода вызывается через публичный метод `validate_mime_type()`.
     *
     * @protected
     *
     * @param string $real_mime_type Реальный MIME-тип файла.
