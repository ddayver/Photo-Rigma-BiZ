...
/**
 * Файл содержит класс Work_Image, который отвечает за работу с изображениями.
 *
 * Этот файл содержит класс `Work_Image`, который реализует интерфейс `Work_Image_Interface`.
 * Класс предоставляет методы для работы с изображениями, включая изменение размеров, вывод через HTTP, обработку
 * отсутствующих изображений, корректировку расширения файла в соответствии с его MIME-типом и логирование ошибок.
 * Публичные методы класса являются редиректами на защищённые методы с суффиксом `_internal`, что обеспечивает более
 * гибкую архитектуру (паттерн "фасад").
 *
 * @author    Dark Dayver
 * @version   0.5.0
 * @since     2025-05-29
 ...
 * @uses      \PhotoRigma\Classes\Work Класс, через который вызываются методы для работы с изображениями.
 ...
 *
 * Этот класс реализует интерфейс `Work_Image_Interface` и предоставляет функционал для работы с изображениями.
 * Он является дочерним классом для `\PhotoRigma\Classes\Work` и наследует его методы. Все методы данного класса
 * рекомендуется вызывать через родительский класс `Work`, так как их поведение может быть непредсказуемым при прямом
 * вызове.
 ...
 * работа с файловой системой и генерация безопасного вывода.
 *
 * @property array $config Конфигурация приложения.
 *
 * @uses \PhotoRigma\Interfaces\Work_Image_Interface Интерфейс, который реализует данный класс.
 */
class Work_Image implements Work_Image_Interface
{
     ...
     * Конструктор класса.
     *
     * Инициализирует объект класса `Work_Image` с переданной конфигурацией. Этот класс является дочерним
     * для \PhotoRigma\Classes\Work.
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Work Родительский класс, который создаёт объект класса Work_Image.
     * @uses \PhotoRigma\Classes\Work_Image::$config Свойство, содержащее конфигурацию приложения.
     */
     ...
     *
     * @uses \PhotoRigma\Classes\Work_Image::$config Свойство, к которому обращается метод.
     */
     ...
     *          Некорректные имена свойств вызовут исключение.
     *
     * @uses \PhotoRigma\Classes\Work_Image::$config Свойство, которое изменяет метод.
     */
    ...
    /**
     * Создаёт директории для категории через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _create_directory_internal().
     * Он создаёт директории для категории. Метод также доступен через метод-фасад `create_directory()` в
     * родительском классе.
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Work_Image::_create_directory_internal() Защищённый метод, реализующий основную логику.
     */
    ...
    /**
     * Создает директории для категории и копирует файлы `index.php`, предварительно проверяя права доступа.
     *
     * Этот защищённый метод выполняет следующие действия:
     * 1. Формирует пути для директорий галереи и миниатюр с использованием констант:
     *    - `GALLERY_DIR`: Директория для галереи.
     *    - `THUMBNAIL_DIR`: Директория для миниатюр.
     ...
     * Метод вызывается через публичный метод-редирект `\PhotoRigma\Classes\Work_Image::create_directory()` и
     * предназначен для использования внутри класса или его наследников.
     *
     * @protected
     *
     * @param string $directory_name Имя директории.
    ...
    /**
     * Корректирует расширение файла в соответствии с его MIME-типом через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _fix_file_extension_internal().
     ...
     *          Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Image::_fix_file_extension_internal() Защищённый метод, выполняющий основную
     *                                                                      логику.
     */
    ...
    /**
     * Корректировка расширения файла в соответствии с его MIME-типом (защищённый метод).
     *
     * Этот защищённый метод выполняет следующие действия:
     ...
     *    - 'image/x-pcx' -> 'pcx'
     * 5. Если расширение отсутствует, добавляет его. Если расширение некорректное, заменяет его на правильное.
     * 6. Логирует случаи добавления или исправления расширения через `log_in_file`.
     * Метод вызывается через публичный метод `fix_file_extension()` и предназначен для использования внутри класса
     * или его наследников.
     *
     * @protected
     *
    ...
    /**
     * Выводит изображение через HTTP через вызов внутреннего метода.
     *
     * Этот публичный метод является обёрткой для защищённого метода _image_attach_internal().
     * Он проверяет существование и доступность файла, определяет его MIME-тип и отправляет содержимое файла через
     * HTTP. Метод также доступен через метод-фасад `image_attach()` в родительском классе.
     *
     * Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется. В противном
     * случае поведение метода может быть непредсказуемым.
     *
     ...
     *          Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Image::_image_attach_internal() Защищённый метод, выполняющий основную логику.
     */
     ...
     *    - Дополнительные заголовки безопасности: `X-Content-Type-Options`, `X-Frame-Options`, `X-XSS-Protection`,
     ...
     * Метод вызывается через публичный метод `image_attach()` и предназначен для использования внутри класса или его
     * наследников.
     *
     * @protected
     *
     ...
     *
     * @uses \PhotoRigma\Include\log_in_file() Записывает сообщение об ошибке в лог-файл.
     * @uses \PhotoRigma\Classes\Work::clean_field() Метод для очистки имени файла.
     */
     ...
     *
     * Этот публичный метод является обёрткой для защищённого метода _image_resize_internal().
     ...
     *          Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Image::_image_resize_internal() Защищённый метод, выполняющий основную логику.
     */
     ...
     *
     * Этот защищённый метод выполняет следующие действия:
     * 1. Проверяет корректность путей к исходному изображению и файлу эскиза с помощью `realpath()` и регулярного
     *    выражения `/^[a-zA-Z0-9\/\.\-_]+$/`.
     * 2. Проверяет существование и доступность исходного изображения для чтения (`file_exists`, `is_readable`).
     ...
     * Метод вызывается через публичный метод `image_resize()` и предназначен для использования внутри класса или его
     * наследников.
     *
     * @protected
     *
     ...
     *          Убедитесь, что пути к файлам и директориям корректны перед вызовом метода.
     *
     * @uses \PhotoRigma\Classes\Work_Image::_calculate_thumbnail_size() Метод для расчёта размеров эскиза.
     * @uses \PhotoRigma\Classes\Work_Image::_process_image_resize_gmagick() Обработка через GraphicsMagick.
     * @uses \PhotoRigma\Classes\Work_Image::_process_image_resize_imagick() Обработка через ImageMagick.
     * @uses \PhotoRigma\Classes\Work_Image::_process_image_resize_gd() Обработка через GD.
     * @uses \PhotoRigma\Include\log_in_file() Записывает сообщение об ошибке в лог-файл.
     */
    protected function _image_resize_internal(string $full_path, string $thumbnail_path): bool
    {
        ...
        if ($full_size[0] > MAX_IMAGE_WIDTH || $full_size[1] > MAX_IMAGE_HEIGHT) {
            throw new RuntimeException(
    ...
    /**
     * Расчёт пропорциональных размеров эскиза на основе реальных размеров изображения и конфигурации.
     *
     ...
     *
     * @uses \PhotoRigma\Classes\Work_Image::$config Массив с конфигурацией приложения.
     */
    ...
    /**
     * Создаёт эскиз изображения с использованием библиотеки GraphicsMagick, включая резервное копирование старого
    ...
    /**
     * Создаёт эскиз изображения с использованием библиотеки ImageMagick, включая резервное копирование старого
     * эскиза и восстановление при ошибках.
     *
     * Этот метод выполняет следующие шаги:
     * 1. Проверяет поддержку MIME-типа через массив разрешённых форматов.
    ...
    /**
     * Создаёт эскиз изображения с использованием библиотеки GD, включая резервное копирование старого эскиза и
     ...
     *          файлов). Убедитесь, что скрипт имеет необходимые права на запись и чтение.
     *          Ограничение памяти (25% от доступной) может быть недостаточным для больших изображений.
     *          Поддержка форматов зависит от текущей версии GD. Например, WebP может быть недоступен.
     *
     * @uses \PhotoRigma\Include\log_in_file() Записывает сообщение об ошибке в лог-файл.
     */
     ...
     *
     * Этот публичный метод является обёрткой для защищённого метода _no_photo_internal().
     * Он формирует массив данных для представления информации об отсутствующем изображении. Метод также доступен
     * через метод-фасад `no_photo()` в родительском классе.
     *
     * Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется. В противном
     * случае поведение метода может быть непредсказуемым.
     *
     ...
     *
     * @warning Метод зависит от корректности данных в конастантах (`SITE_URL`, `GALLERY_DIR`, `THUMBNAIL_DIR`). Если
     *          эти параметры некорректны, результат может быть непредсказуемым. Вызывать этот метод напрямую, минуя
     *          метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Image::_no_photo_internal() Защищённый метод, выполняющий основную логику.
     */
     ...
     *
     * Этот защищённый метод формирует массив данных, который используется для представления информации об
     ...
     * - `GALLERY_DIR`: Директория для хранения изображений.
     * - `THUMBNAIL_DIR`: Директория для хранения эскизов.
     * Массив содержит предопределённые значения для всех ключей, таких как `'url'`, `'thumbnail_url'`, `'name'`,
     * `'description'` и другие. Значения по умолчанию используются для отсутствующих данных.
     * Этот метод является защищённым и предназначен для использования внутри класса или его наследников. Основная
     * логика метода вызывается через публичный метод `no_photo()`.
     *
     * @protected
     *
     * @return array Массив данных об изображении или его отсутствии:
     ...
     *
     * @warning Метод зависит от корректности данных в конастантах (`SITE_URL`, `GALLERY_DIR`, `THUMBNAIL_DIR`). Если
     *          эти параметры некорректны, результат может быть непредсказуемым.
     */
     ...
     *
     * Этот публичный метод является обёрткой для защищённого метода _remove_directory_internal().
     * Он удаляет директорию и всё её содержимое. Метод также доступен через метод-фасад `remove_directory()` в
     * родительском классе.
     *
     * Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется. В противном
     * случае поведение метода может быть непредсказуемым.
     ...
     *          Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Image::_remove_directory_internal() Защищённый метод, выполняющий основную
     *                                                                    логику удаления.
     */
     ...
     * Метод вызывается через публичный метод-редирект `\PhotoRigma\Classes\Work_Image::remove_directory()` и
     * предназначен для использования внутри класса или его наследников.
     *
     * @protected
     *
     ...
     *
     * Этот публичный метод является обёрткой для защищённого метода _size_image_internal().
     * Он рассчитывает ширину и высоту эскиза на основе реальных размеров изображения и конфигурационных параметров
     * (`temp_photo_w` и `temp_photo_h`). Метод также доступен через метод-фасад `size_image()` в родительском классе.
     *
     * Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется. В противном
     * случае поведение метода может быть непредсказуемым.
     *
     ...
     *          Вызывать этот метод напрямую, минуя метод-фасад родительского класса, крайне не рекомендуется.
     *
     * @uses \PhotoRigma\Classes\Work_Image::_size_image_internal() Защищённый метод, выполняющий основную логику.
     */
     ...
     * Этот метод является защищённым и предназначен для использования внутри класса или его наследников. Основная
     * логика метода вызывается через публичный метод `size_image()`.
     *
     * @protected
     *
     * @param string $path_image Путь к файлу изображения.
     ...
     *          Если эти параметры некорректны, результат может быть непредсказуемым.
     *
     * @uses \PhotoRigma\Classes\Work_Image::_calculate_thumbnail_size() Метод, используемый для расчёта размера эскиза.
     */
