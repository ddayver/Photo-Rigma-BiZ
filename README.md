...
- **СУБД (Система управления базами данных)**:
  ...
  - **SQLite** *(тестовая поддержка)*: Версия **3+**. Требуется поддержка **FTS5**.
...
- **Для SQLite**:
  SQLite не требует создания базы данных через SQL. Просто укажите путь к файлу базы данных в конфигурации. Если файл не существует, он будет создан автоматически при первом подключении.
  Убедитесь, что директория, в которой находится файл, доступна для записи.
...
$config['gallery_dir']   = 'gallery'; // Папка для хранения фотографий
$config['thumbnail_dir'] = 'thumbnail'; // Папка для хранения эскизов
$config['avatar_dir']    = 'avatar'; // Папка для хранения аватаров
...
> - Если используете SQLite:
    >   - Установите параметр `dbtype` на `'sqlite'`.
